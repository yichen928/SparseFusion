...
import os
import pickle
from vis_util import project_pts_on_img, draw_bboxes_2d
import cv2
import matplotlib
...
img_query_heatmap = torch.load("vis/img_query_pos_heatmap.pt")
img_query_heatmap_label = torch.load("vis/img_query_label.pt")
img_query_pos = torch.load("vis/img_query_pos.pt")
img_query_label = torch.load("vis/img_query_pred.pt")
img_query_label = torch.max(img_query_label, dim=1)[1]
...
img_query_heatmap_np = img_query_heatmap.detach().cpu().numpy()
img_query_heatmap_label_np = img_query_heatmap_label.detach().cpu().numpy()
img_query_pos_np = img_query_pos.detach().cpu().numpy()
img_query_label_np = img_query_label.detach().cpu().numpy()
...
for sample_idx in range(bs):
    ...
    for view_idx in range(num_view):
        ...
        on_the_image_2d = img_query_view_3d_np[sample_idx] == view_idx
        on_the_image = np.logical_and(on_the_image_2d, pos_mask)
        ...
        width = img_view.shape[1]
        height = img_view.shape[0]

        heatmap_pos_view = img_query_heatmap_np[sample_idx, on_the_image]
        heatmap_pos_view = sigmoid(heatmap_pos_view)
        heatmap_pos_view[..., 0] = heatmap_pos_view[..., 0] * width
        heatmap_pos_view[..., 1] = heatmap_pos_view[..., 1] * height
        heatmap_label_view = img_query_heatmap_label_np[sample_idx, on_the_image]
        img_query_pos_view = img_query_pos_np[sample_idx, on_the_image]
        img_query_pos_view = sigmoid(img_query_pos_view)
        img_query_pos_view[..., 0] = img_query_pos_view[..., 0] * width
        img_query_pos_view[..., 1] = img_query_pos_view[..., 1] * height
        img_query_label_view = img_query_label_np[sample_idx, on_the_image]

        # new_img = draw_lidar_bbox3d_on_img(lidar_bbox_3d, img_view, img_metas[sample_idx]['lidar2img'][view_idx], img_metas)

        inds_2d_view = img_labels_2d[:, 1] == view_idx
        gt_bbox_2d_view = gt_bbox_2d_np[sample_idx][inds_2d_view]
        gt_label_2d_view = gt_label_2d_np[sample_idx][inds_2d_view]
        ...
        if gt_bbox_2d_view.shape[0] > 0:
            ...
            img_view = draw_bboxes_2d(gt_bbox_2d_view, img_view, labels=gt_label_2d_view)

        # import pdb
        # pdb.set_trace()
        for i in range(heatmap_pos_view.shape[0]):
            img_view = cv2.putText(img_view, '%d' % heatmap_label_view[i], (round(heatmap_pos_view[i][0]), round(heatmap_pos_view[i][1])), cv2.FONT_HERSHEY_SIMPLEX,
                                   1, (0, 255, 0), 3, cv2.LINE_AA)
            img_view = cv2.putText(img_view, '%d' % img_query_label_view[i], (round(img_query_pos_view[i][0]), round(img_query_pos_view[i][1])), cv2.FONT_HERSHEY_SIMPLEX,
                                   1, (255, 0, 0), 3, cv2.LINE_AA)

        cv2.imwrite("vis/img_b%d_v%d.png"%(sample_idx, view_idx), img_view)
