...
def parse_args():
    ...
    parser.add_argument("--score_thre", type=float, default=0.01)
...
def main():
    ...
    with open(args.checkpoint, 'rb') as f:
        raw_predictions=json.load(f)['results']

    predictions = {}
    for key in raw_predictions:
        preds = []
        for item in raw_predictions[key]:
            if item['detection_score'] > args.score_thre:
                preds.append(item)
