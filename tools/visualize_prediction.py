...
fusion_cls_tensor = torch.load("vis/fusion_score.pt")
fusion_cls, fusion_cls_label = torch.max(fusion_cls_tensor, dim=1)
...
for bid in range(bs):
    ...
    pred_bbox_i = torch.load("vis/bboxes_tensor_%d.pt"%bid, map_location='cpu')[200:]

    pred_score = fusion_cls[bid].sigmoid()
    pred_label = fusion_cls_label[bid]

    fuse_lidar_bbox_i = pred_bbox_i[:200]
    fuse_cam_bbox_i = pred_bbox_i[200:]
    pred_lidar_score = pred_score[:200]
    pred_cam_score = pred_score[200:]
    pred_lidar_label = pred_label[:200]
    pred_cam_label = pred_label[200:]

    lidar_mask = pred_lidar_score > threshold
    cam_mask = pred_cam_score > threshold
    fuse_lidar_bbox_i = fuse_lidar_bbox_i[lidar_mask]
    fuse_cam_bbox_i = fuse_cam_bbox_i[cam_mask]

    fuse_lidar_bbox_i = LiDARInstance3DBoxes(fuse_lidar_bbox_i, box_dim=fuse_lidar_bbox_i.shape[-1], origin=(0.5, 0.5, 0.0)).convert_to(Box3DMode.LIDAR)
    fuse_cam_bbox_i = LiDARInstance3DBoxes(fuse_cam_bbox_i, box_dim=fuse_cam_bbox_i.shape[-1], origin=(0.5, 0.5, 0.0)).convert_to(Box3DMode.LIDAR)
    ...
    for view_id in range(view_num):
        ...
        if fuse_lidar_bbox_i.tensor.shape[0] > 0:
            view_img = our_draw_lidar_bbox3d_on_img(fuse_lidar_bbox_i, view_img, lidar2img_rt, img_metas[bid], view_id=view_id, color=(255, 0, 0))

        if fuse_cam_bbox_i.tensor.shape[0] > 0:
            view_img = our_draw_lidar_bbox3d_on_img(fuse_cam_bbox_i, view_img, lidar2img_rt, img_metas[bid], view_id=view_id, color=(0, 255, 0))
