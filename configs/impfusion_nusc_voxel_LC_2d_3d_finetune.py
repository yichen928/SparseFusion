...
train_pipeline = [
    ...
    dict(type='Collect3D', keys=['points', 'img', 'gt_bboxes_3d', 'gt_labels_3d', 'gt_bboxes', 'gt_labels'])
]
test_pipeline = [
    dict(
        type='LoadPointsFromFile',
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            ...
            ann_file=data_root + '/nuscenes_infos_train_with_proj2d_20pc.pkl',
            load_interval=1,
    ...
    val=dict(
        ...
        ann_file=data_root + '/nuscenes_infos_val_with_proj2d.pkl',
        load_interval=1,
    ...
    test=dict(
        ...
        box_type_3d='LiDAR'))
model = dict(
    type='ImplicitFusionDetector2D_3D',
    freeze_img=True,
    freeze_img_head=True,
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
...
load_from = 'work_dirs/impfusion_nusc_voxel_LC_2d_3d_PtsQueryImageKey_gridCoord_freezePoint_finetuneImage_ProjLayerNorm_raybug_2Dsupbug/latest.pth'
resume_from = None
workflow = [('train', 1)]
gpu_ids = range(0, 8)
