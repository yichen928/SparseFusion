...
from mmdet3d.models.utils import PositionEmbeddingLearned, TransformerDecoderLayer, MultiheadAttention, FFN, PointTransformerL3D, ImageTransformerL3D, FusionTransformerL3D

@HEADS.register_module()
class ImplicitHeadL3D(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
        ...
        super(ImplicitHeadL3D, self).__init__()
        ...
        if share_prediction_heads:
            ...
            fusion_prediction_heads = FFN(hidden_channel*2, heads, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias)
        ...
        if with_pts:
            ...
            self.point_transformer = PointTransformerL3D(
        ...
        if with_img:
            ...
            self.img_transformer = ImageTransformerL3D(
        ...
        if with_pts and with_img:
            fusion_query_pos_embed = PositionEmbeddingLearned(2, hidden_channel*2)
            fusion_key_pos_embed = PositionEmbeddingLearned(2, hidden_channel*2)
            self.fusion_transformer = FusionTransformerL3D(
                hidden_channel=hidden_channel*2, num_heads=num_heads, num_decoder_layers=num_fusion_decoder_layers, test_cfg=test_cfg,
                prediction_heads=fusion_prediction_heads, ffn_channel=ffn_channel, dropout=dropout, activation=activation,
                query_pos=fusion_query_pos_embed, key_pos=fusion_key_pos_embed
            )
            self.pts_projection = self.build_projection()
            self.img_projection = self.build_projection()
    ...
    def build_projection(self):
        if self.num_projection_layers == 0:
            return nn.Identity()
        layers = []
        for i in range(self.num_projection_layers-1):
            layers.append(nn.Conv1d(self.hidden_channel, self.hidden_channel, kernel_size=1))
            layers.append(nn.BatchNorm1d(self.hidden_channel))
