...
class HeatmapImgProj(nn.Module):
    def __init__(self, hidden_channel, bias):
        super(HeatmapImgProj, self).__init__()
        self.conv3 = ConvModule(
            hidden_channel, hidden_channel, kernel_size=(1, 3), padding=(0, 1), bias=bias,
            conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d')
        )
        self.conv9 = ConvModule(
            hidden_channel, hidden_channel, kernel_size=(1, 9), padding=(0, 4), bias=bias,
            conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d')
        )

    def forward(self, x):
        return self.conv3(x) + self.conv9(x)


class CenterPooling(nn.Module):
    def __init__(self, hidden_channel, h_conv, v_conv):
    ...
    def forward(self, feat):
        # [bs*num_views, C, H, W]
        h_feat = self.h_conv(feat)
        v_feat = self.v_conv(feat)

        h_feat_max = torch.max(h_feat, dim=-1, keepdim=True)[0]  # [bs*num_views, C, H, 1]
        v_feat_max = torch.max(v_feat, dim=-2, keepdim=True)[0]  # [bs*num_views, C, 1, W]

        feat_max = h_feat_max + v_feat_max  # [bs*num_views, C, H, W]
        return feat_max


@HEADS.register_module()
class ImplicitHead2D_3D_CrossProj(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
                 pts_smca=False,
        ...
        super(ImplicitHead2D_3D_CrossProj, self).__init__()
        ...
        self.img_heatmap_trick = img_heatmap_trick
        self.cross_heatmap_trick = cross_heatmap_trick
        self.pos_3d = pos_3d
        self.fuse_self = fuse_self
        self.save_for_test = save_for_test
        if self.save_for_test is not None:
            with open(self.save_for_test, "rb") as file:
                self.saved_data = pickle.load(file, encoding="bytes")
        ...
        if with_pts and with_img:
            if self.pe_norm == 'bn':
                ...
                if interact_with_img:
                    depth_img_key_pos_embed = PositionEmbeddingLearned(2, hidden_channel)
                else:
                    depth_img_key_pos_embed = None
            else:
                ...
                if interact_with_img:
                    depth_img_key_pos_embed = PositionEmbeddingLearnedwoNorm(2, hidden_channel)
                else:
                    depth_img_key_pos_embed = None
            ...
            self.depth_transformer = DepthTransformer2D_3D(
                ...
                depth_key_3d=depth_key_3d, img_key_pos=depth_img_key_pos_embed
            )
            ...
            if self.initialize_by_heatmap and self.cross_heatmap:
                if cat_heatmap:
                    ...
                    if heatmap_ms_proj:
                        self.heatmap_img_proj = HeatmapImgProj(hidden_channel, bias)
                    else:
                        self.heatmap_img_proj = ConvModule(
                            hidden_channel, hidden_channel, kernel_size=1, bias=bias,
                            conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d'),
                        )
                    ...
                    self.cross_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, dcn=cross_heatmap_dcn, layer_num=cat_heatmap_layer)

                    if cross_heatmap_trick == 'msfusion':
                        self.cross_msfusion = MSFusion(hidden_channel, dict(type='BN2d'))
                    else:
                        assert cross_heatmap_trick == 'none'

                elif colattn_heatmap:
                    self.heatmap_pts_proj = ConvModule(
                        hidden_channel, hidden_channel, kernel_size=1, bias=bias,
                        conv_cfg=dict(type='Conv2d'), norm_cfg=dict(type='BN2d'),
                        act_cfg=None
                    ...
                    if self.colattn_pos:
                        ...
                        #     colattn_key_pos = PositionEmbeddingLearnedwoNorm(1, hidden_channel)
        ...
        if self.initialize_by_heatmap:
            ...
            if img_heatmap_trick == 'centerpool':
                h_conv = ConvModule(
                ...
                self.center_pool = CenterPooling(hidden_channel, h_conv=h_conv, v_conv=v_conv)
                self.img_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, layer_num=img_heatmap_layer, dcn=img_heatmap_dcn)
            elif img_heatmap_trick == 'msca':
                self.msca = MSCABlock(dim=hidden_channel, mlp_ratio=4, drop_path=0.1, norm_cfg=dict(type='BN2d'), act_layer=nn.GELU)
                self.img_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, kernel_size=3, layer_num=img_heatmap_layer, dcn=img_heatmap_dcn)
            elif img_heatmap_trick == 'msfusion':
                self.img_msfusion = MSFusion(hidden_channel, dict(type='BN2d'))
                self.img_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, kernel_size=3, layer_num=img_heatmap_layer, dcn=img_heatmap_dcn)
            elif img_heatmap_trick == 'none':
                self.img_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, layer_num=img_heatmap_layer, dcn=img_heatmap_dcn)
            else:
                raise NotImplementedError
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.with_pts:
            ...
            if self.initialize_by_heatmap:
                if self.with_img and self.cross_heatmap:
                    ...
                    if self.img_heatmap_trick == 'msca':
                        img_feat_cross = self.msca(img_feat_cross.clone())
                else:
                    img_feat_cross = None
        ...
        if self.with_img:
            ...
            if self.initialize_by_heatmap:
                ...
                if self.img_heatmap_trick == 'msca':
                    img_feat_heatmap = img_feat_cross.clone()
        ...
        torch.cuda.synchronize()
        t4 = time.time()
        print("point transformer init: %.4f"%(t4-t3))
        ...
        torch.cuda.synchronize()
        t5 = time.time()
        print("img transformer: %.4f"%(t5-t4))
        ...
        if self.with_img and self.with_pts:
            ...
            torch.cuda.synchronize()
            t6 = time.time()
            print("depth transformer: %.4f" % (t6 - t5))
    ...
    def generate_heatmap_colattn(self, lidar_feat, img_feat, img_metas):
        ...
        if batch_size > 1 or self.save_for_test is None:
        ...
        for sample_idx in range(batch_size):
            ...
            lidar_feat_view = torch.zeros([img_feat.shape[-1], self.hidden_channel, 75]).to(lidar_feat_output.device)  # [red_img_w, C, max_num]
            ...
            for view_idx in range(self.num_views):
                ...
                if batch_size > 1 or self.save_for_test is None:
                    ...
                    # view_dump_data['center_xs'] = center_xs
                    # view_dump_data['index'] = index
                    # view_dump_data['on_the_image'] = on_the_image
                    # view_dump_data['depth'] = current_view_depth
                    # dump_data.append(view_dump_data)

                else:
                    center_xs = self.saved_data[view_idx]['center_xs']
                    index = self.saved_data[view_idx]['index']
                    on_the_image = self.saved_data[view_idx]['on_the_image']
                    current_view_depth = self.saved_data[view_idx]['depth']
                    max_num = torch.max(index) + 1
                ...
                if self.colattn_pos:
                    ...
                    output = self.col_decoder(lidar_feat_view[...,:max_num], img_feat_view, None, None) # [img_w, C, max_num]

                lidar_feat_output[sample_idx, :, on_the_image] = output[center_xs, :, index].clone().transpose(1, 0)

            # with open("dump_data.pkl", "wb") as file:
    ...
    def generate_heatmap_cat(self, lidar_feat, img_feat, img_metas):
        # img_feat [bs*num_view, C, img_h, img_w]
        # lidar_feat [BS, C, H, W]
        img_feat = self.heatmap_img_proj(img_feat)
        lidar_feat = self.heatmap_pts_proj(lidar_feat)

        if self.cross_heatmap_trick == 'msfusion':
            img_feat = self.cross_msfusion(img_feat)

        batch_size = lidar_feat.shape[0]
        H, W = lidar_feat.shape[2], lidar_feat.shape[3]
        # lidar_feat = lidar_feat.reshape(batch_size, self.hidden_channel, H*W)
        img_w = img_feat.shape[3]

        img_feat_collapsed = torch.max(img_feat, dim=2)[0]  # (bs*num_view, C, img_w)
        img_feat_collapsed = img_feat_collapsed.reshape(batch_size, self.num_views, self.hidden_channel, img_w)  # (bs, num_view, C, img_w)
        # zero_pad = torch.zeros([batch_size, self.num_views, self.hidden_channel, 1]).type_as(img_feat_collapsed)
        # img_feat_collapsed_pad = torch.cat([img_feat_collapsed, zero_pad], dim=-1)

        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(img_feat_collapsed.device)
        query_pos_realmetric = bev_pos.permute(0, 2, 1) * self.test_cfg['out_size_factor'] * \
                               self.test_cfg['voxel_size'][0] + self.test_cfg['pc_range'][0]  # (bs, 2, H*W)

        img_feat_cat = torch.zeros_like(lidar_feat)
        img_feat_cat = img_feat_cat.reshape(batch_size, self.hidden_channel, H*W)

        for sample_idx in range(batch_size):
            query_pos_3d = torch.cat([query_pos_realmetric[sample_idx], torch.ones_like(query_pos_realmetric[sample_idx][:1])], dim=0).detach().clone()  # (3, N)

            lidar2img_rt = torch.Tensor(img_metas[sample_idx]['lidar2img']).to(img_feat_cat.device)
            img_scale_factor = (
                lidar2img_rt.new_tensor(img_metas[sample_idx]['scale_factor'][:2] if 'scale_factor' in img_metas[sample_idx].keys() else [1.0, 1.0])
            )
            ...
            for view_idx in range(self.num_views):
                ...
                on_the_image = (center_xs >= 0) & (center_xs < img_feat_collapsed.shape[-1])
                center_xs = center_xs[on_the_image]

                img_feat_cat[sample_idx, :, on_the_image] = torch.maximum(img_feat_cat[sample_idx, :, on_the_image], img_feat_collapsed[sample_idx, view_idx, :, center_xs])
                # img_feat_cat[sample_idx, :, on_the_image] = img_feat_collapsed[sample_idx, view_idx, :, center_xs].clone()
        img_feat_cat = img_feat_cat.reshape(batch_size, self.hidden_channel, H, W)
        feat_cat = torch.cat([lidar_feat, img_feat_cat], dim=1)
        feat_cat = self.reduce_conv(feat_cat)
        return self.cross_heatmap_head(feat_cat)
    ...
    def generate_heatmap(self, lidar_feat, batch_size, img_metas, img_feat=None):
        ...
        if img_feat is None:
            ...
            if self.cat_heatmap:
                dense_heatmap_cross = self.generate_heatmap_cat(lidar_feat, img_feat, img_metas)
            elif self.colattn_heatmap:
    ...
    def generate_heatmap_img(self, img_feat, batch_size):
        # if self.img_heatmap_trick == 'centerpool':
        #     img_feat = self.center_pool(img_feat)
        # elif self.img_heatmap_trick == 'msca':
        #     img_feat = self.msca(img_feat)

        if self.img_heatmap_trick == 'msfusion':
            img_feat = self.img_msfusion(img_feat)
    ...
    def get_targets_single_2d(self, gt_bboxes, gt_labels, gt_centers_2d, preds_dict, img_metas, batch_idx):
        ...
        for idx_layer in range(self.num_img_decoder_layers):
            ...
            # zero_bboxes = torch.zeros((0, 4)).type_as(gt_bboxes)
            # zero_labels = torch.zeros((0, 2)).type_as(gt_labels)
            # zero_centers_2d = torch.zeros((0, 3)).type_as(gt_centers_2d)
            # zero_offsets = torch.zeros((0, 4)).type_as(gt_offsets)
            # import pdb
            # pdb.set_trace()
            # assign_result = self.bbox_assigner_2d.assign(bboxes_tensor_layer, score_layer, centers_layer, offsets_layer, view_layer, zero_bboxes, zero_labels, zero_centers_2d[..., :2], zero_offsets, img_w=self.test_cfg['img_scale'][0], img_h=self.test_cfg['img_scale'][1])
            # import pdb
            # pdb.set_trace()
        ...
        if self.with_img and self.supervision2d:
            ...
            for idx_layer in range(self.num_img_decoder_layers):
                ...
                if idx_layer == self.num_img_decoder_layers - 1:
                    ...
                    # pred_depth_prob = F.softmax(pred_depth, dim=-1)
                    # depth_targets = F.one_hot(depth_labels, num_classes=self.num_depth_class).float()
                    # depth_weights = depth_weights.reshape(-1,1).repeat(1, self.num_depth_class).float()
                    # loss_depth = F.binary_cross_entropy(pred_depth_prob, depth_targets, weight=depth_weights, reduction='sum') / max(num_pos_layer_2d[idx_layer], 1)
