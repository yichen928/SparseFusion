...
from mmdet3d.models.utils import FFN, TransformerDecoderLayer, PositionEmbeddingLearned, PositionEmbeddingLearnedwoNorm, PositionEmbeddingLearnedMulti, PointTransformer2D_3D_PETR, ImageTransformer2D_3D_PETR, FusionTransformer2D_3D_PETR, PointProjection, ImageProjection, ProjectionL2Norm, ProjectionLayerNorm, PETR_Heads
...
def bev2world(pos, x_max, y_max, pc_range):
    max_xy = torch.Tensor([x_max, y_max]).to(pos.device).view(1, 1, 2)
    normal_pos = pos / max_xy
    min_range = torch.Tensor([pc_range[0], pc_range[1]]).to(pos.device).view(1, 1, 2)
    max_range = torch.Tensor([pc_range[3], pc_range[4]]).to(pos.device).view(1, 1, 2)
    pos = normal_pos * (max_range - min_range) + min_range
    return pos

@HEADS.register_module()
class ImplicitHead2D_3D_PETR(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
                 pts_smca=True,
        ...
        super(ImplicitHead2D_3D_PETR, self).__init__()
        ...
        heads3d.update(dict(heatmap=(self.num_classes, 3)))
        pts_prediction_heads = PETR_Heads(hidden_channel, heads3d)
        fusion_prediction_heads = PETR_Heads(hidden_channel, heads3d)

        heads2d = dict(bbox=(4, 2), cls=(self.num_classes, 2))
        img_prediction_heads = PETR_Heads(hidden_channel, heads2d)

        if with_pts:
            if self.pe_norm == 'bn':
                pts_query_pos_embed = PositionEmbeddingLearned(3, hidden_channel)
                pts_key_pos_embed = PositionEmbeddingLearned(2, hidden_channel)
            else:
                pts_query_pos_embed = PositionEmbeddingLearnedwoNorm(3, hidden_channel)
                pts_key_pos_embed = PositionEmbeddingLearnedwoNorm(2, hidden_channel)
            self.point_transformer = PointTransformer2D_3D_PETR(
        ...
        if with_img:
            ...
            self.img_transformer = ImageTransformer2D_3D_PETR(
        ...
        if with_pts and with_img:
            ...
            self.fusion_transformer = FusionTransformer2D_3D_PETR(
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.with_pts:
            x_size = self.test_cfg['grid_size'][0] // self.test_cfg['out_size_factor']
            y_size = self.test_cfg['grid_size'][1] // self.test_cfg['out_size_factor']
            # pts_query_pos = normalize_pos(pts_query_pos, x_size, y_size)
            # bev_pos = normalize_pos(bev_pos, x_size, y_size)
            pts_query_pos = bev2world(pts_query_pos, x_size, y_size, self.test_cfg['pc_range'])
            bev_pos = bev2world(bev_pos, x_size, y_size, self.test_cfg['pc_range'])
        ...
        # new_res['center'][:, 0] = new_res['center'][:, 0].sigmoid() * (self.test_cfg['pc_range'][3] - self.test_cfg['pc_range'][0]) + self.test_cfg['pc_range'][0]
        # new_res['center'][:, 1] = new_res['center'][:, 1].sigmoid() * (self.test_cfg['pc_range'][4] - self.test_cfg['pc_range'][1]) + self.test_cfg['pc_range'][1]
        # new_res['height'] = new_res['height'].sigmoid() * (self.test_cfg['pc_range'][5] - self.test_cfg['pc_range'][2]) + self.test_cfg['pc_range'][2]
