...
from mmdet3d.models.utils import FFN, TransformerDecoderLayer, PositionEmbeddingLearned, PositionEmbeddingLearnedwoNorm,\
    ...
    FusionTransformer2D_3D_Self, ASPP, DepthEstimation
...
#     def __init__(self, input_channel, hidden_channel, kernel_size=3,  stride=1, padding=1, require_act=True):
#         super().__init__()
#         self.conv = nn.Conv2d(input_channel, hidden_channel, kernel_size=kernel_size, stride=stride, padding=padding)
...
class ImplicitHead2D_3D_MS_Deform(nn.Module):
    def __init__(self,
        ...
        self.depth_transfer = depth_transfer
        ...
        if self.depth_reg:
            heads_depth = dict(depth=(1, img_reg_layer, img_reg_bn))
        else:
            heads_depth = dict(depth=(self.num_depth_class, img_reg_layer))
        if self.depth_transfer:
            heads_depth.update(proj_center=(2, img_reg_layer, img_reg_bn))
        if extra_class_sup:
        ...
        if with_pts and with_img:
            ...
            if self.depth_transfer:
                ...
                self.depth_transformer = DepthEstimation(
                    hidden_channel=hidden_channel, prediction_heads=depth_prediction_heads, test_cfg=test_cfg,
                    dbound=dbound, use_camera=use_camera, extra_camera=extra_camera
                )
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.with_pts:
            ...
            if self.initialize_by_heatmap:
                ...
                heatmap, dense_heatmap, pts_top_proposals_class, pts_top_proposals_index, fuse_lidar_feat_flatten = self.generate_heatmap(lidar_feat.clone(), voxel_height, batch_size, img_metas, img_feat_cross)
        ...
        if self.with_img:
            ...
            img_query_feat, normal_img_query_pos, img_ret_dicts = self.img_transformer(img_query_feat, normal_img_query_pos, img_query_view, img_feats, normal_img_feats_pos_stack, img_metas)
        ...
        if self.with_img and self.with_pts:
            ...
            if self.depth_transfer:
                ...
                img_query_feat, img_query_pos_bev, depth_ret_dicts, extra_mul = self.depth_transformer(img_query_feat, normal_img_query_pos, img_query_view, img_metas)
    ...
    def generate_heatmap_colattn(self, lidar_feat, img_feat, voxel_height, img_metas):
        ...
        lidar_feat_output = torch.zeros(batch_size, self.hidden_channel+1, H*W).to(lidar_feat.device)
        lidar_feat_output[:, :-1] = lidar_feat_flatten.clone()  
        ...
        for sample_idx in range(batch_size):
            ...
            for view_idx in range(self.num_views):
                ...
                # camera_pos = -img_metas[sample_idx]['lidar2cam_r'][view_idx].T@img_metas[sample_idx]['lidar2cam_t'][view_idx]
                # camera_pos = torch.from_numpy(camera_pos).type_as(lidar_feat_view)
                # camera_pos_lidar = camera_pos[None, :].repeat(lidar_feat_pos_view.shape[0], 1)
                #
                # flip = torch.zeros(3).type_as(lidar_feat_pos_view)
                # if 'pcd_horizontal_flip' in img_metas[sample_idx] and img_metas[sample_idx]['pcd_horizontal_flip'] == True:
                #     flip[0] = 1
                # if 'pcd_vertical_flip' in img_metas[sample_idx] and img_metas[sample_idx]['pcd_vertical_flip'] == True:
                #     flip[1] = 1
                # if 'image_flip' in img_metas[sample_idx] and img_metas[sample_idx]['image_flip'][view_idx] == True:
                #     flip[2] = 1
                # flip = flip[None, :].repeat(lidar_feat_pos_view.shape[0], 1)
                #
                # lidar_feat_pos_view = torch.cat([lidar_feat_pos_view, depth[:, None], query_pos_3d_view, camera_pos_lidar, flip], dim=1)

                # camera_pos_img = camera_pos[None, None, :].repeat(1, normal_img_feats_pos_stack.shape[1], 1)
                # normal_img_feats_pos_stack_view = torch.cat([normal_img_feats_pos_stack, camera_pos_img], dim=2)
    ...
    def generate_heatmap(self, lidar_feat, voxel_height, batch_size, img_metas, img_feat=None):
        ...
        if img_feat is None:
            ...
            dense_heatmap_cross, fuse_lidar_feature_flatten = self.generate_heatmap_colattn(lidar_feat, img_feat, voxel_height, img_metas)
            if self.cross_only:
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_pts_centers_2d, gt_img_centers_2d, img_metas, preds_dicts, **kwargs):
        ...
        if self.with_img and self.supervision2d:
            ...
            for idx_layer in range(self.num_img_decoder_layers):
                ...
                layer_loss_center_2d = self.loss_bbox_2d(preds_2d[...,:2], layer_bbox_targets_2d[...,:2], layer_reg_weights_2d[...,:2], avg_factor=max(num_pos_layer_2d[idx_layer], 1))
                layer_loss_offset_2d = self.loss_bbox_2d(preds_2d[...,2:], layer_bbox_targets_2d[...,2:], layer_reg_weights_2d[...,2:], avg_factor=max(num_pos_layer_2d[idx_layer], 1))
