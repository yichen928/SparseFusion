...
from mmdet3d.models.utils import FFN, TransformerDecoderLayer, PositionEmbeddingLearned, PositionEmbeddingLearnedwoNorm, \
    ...
    ProjectionL2Norm, ProjectionLayerNorm, MSCABlock, ImageTransformer2D_3D_MS, MSFusion, \
    FusionTransformer2D_3D_Self, ASPP
...
class ImplicitHead2D_3D_MS_Deform(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
                 pts_smca=False,
        ...
        super(ImplicitHead2D_3D_MS_Deform, self).__init__()
        ...
        if merge_head_2d:
            ...
            heads2d = dict(center_2d=(2, img_reg_layer, img_reg_bn), offset=(4, img_reg_layer, img_reg_bn),
                           cls=(self.num_classes, 2))
        ...
        if with_pts:
            ...
            self.point_transformer = PointTransformer2D_3D(
                pts_smca=pts_smca, hidden_channel=hidden_channel, num_heads=num_heads,
                num_decoder_layers=num_pts_decoder_layers,
                prediction_heads=pts_prediction_heads, ffn_channel=ffn_channel, dropout=dropout, activation=activation,
                test_cfg=test_cfg,
        ...
        if with_img:
            ...
            self.img_transformer = ImageTransformer2D_3D_MS(
                hidden_channel=hidden_channel, num_heads=num_heads, num_decoder_layers=num_img_decoder_layers,
                out_size_factor_img=out_size_factor_img,
                num_views=num_views, prediction_heads=img_prediction_heads, ffn_channel=ffn_channel, dropout=dropout,
                activation=activation, test_cfg=test_cfg,
        ...
        if with_pts and with_img:
            ...
            fusion_query_pos_embed = [PositionEmbeddingLearned(2, hidden_channel) for _ in
                                      range(self.num_fusion_decoder_layers)]
            if self.pos_3d:
                fusion_key_pos_embed = [PositionEmbeddingLearned(3, hidden_channel) for _ in
                                        range(self.num_fusion_decoder_layers)]
            else:
                fusion_key_pos_embed = [PositionEmbeddingLearned(2, hidden_channel) for _ in
                                        range(self.num_fusion_decoder_layers)]
            ...
            if fuse_self:
                self.fusion_transformer = FusionTransformer2D_3D_Self(
                    ...
                    key_pos=fusion_query_pos_embed,
            ...
            #     if self.colattn_pos:
            #         colattn_query_pos = PositionEmbeddingLearnedwoNorm(128, hidden_channel)
            #         colattn_key_pos = PositionEmbeddingLearnedwoNorm(128, hidden_channel)
            ...
            if self.initialize_by_heatmap and self.cross_heatmap:
                ...
                self.cross_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, dcn=cross_heatmap_dcn,
                                                             layer_num=cross_heatmap_layer)
                if self.colattn_pos:
                    colattn_query_pos = PositionEmbeddingLearnedwoNorm(128, hidden_channel)
                    colattn_key_pos = PositionEmbeddingLearnedwoNorm(128, hidden_channel)
        ...
        if self.initialize_by_heatmap:
            ...
            self.img_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, layer_num=img_heatmap_layer,
                                                       dcn=img_heatmap_dcn)
            ...
            self.pts_query_pos = nn.Parameter(
                torch.rand([1, self.num_proposals, 2]) * torch.Tensor([x_size, y_size]).reshape(1, 1, 2),
                requires_grad=learnable_query_pos)

            self.img_query_feat = nn.Parameter(torch.randn(1, hidden_channel, self.num_img_proposals))
            self.img_query_pos = nn.Parameter(torch.rand([1, self.num_img_proposals, 2]),
                                              requires_grad=learnable_query_pos)
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.with_img:
            for i in range(self.level_num):
                img_feat = self.shared_conv_img(img_inputs[i])
                img_feat = img_feat + self.level_pos[i].reshape(1, self.level_pos[i].shape[0], 1, 1)
                img_feats.append(img_feat)

        if self.with_pts:
            inputs, voxel_height = inputs[:, :-1], inputs[:, -1]
            ...
            if self.initialize_by_heatmap:
                ...
                heatmap, dense_heatmap, pts_top_proposals_class, pts_top_proposals_index, fuse_lidar_feat_flatten = self.generate_heatmap(
                    lidar_feat.clone(), voxel_height, batch_size, img_metas, img_feat_cross)
                ...
                one_hot = F.one_hot(pts_top_proposals_class, num_classes=self.num_classes).permute(0, 2,
                                                                                                   1)  # [BS, num_classes, num_proposals]
                ...
                pts_query_pos = self.pts_query_pos.repeat(batch_size, 1, 1).to(
        ...
        if self.with_img:
            ...
            for lvl in range(self.level_num):
                ...
                img_feat_pos = self.create_2D_grid(h, w).to(lidar_feat.device)  # (1, h*w, 2)
            ...
            if self.initialize_by_heatmap:
                ...
                if self.img_heatmap_stop_grad:
                    for lvl in range(self.level_num):
                        img_feats_heatmap.append(img_feats[lvl].detach().clone())
                else:
                    for lvl in range(self.level_num):
                ...
                img_feat_stack = img_feat_stack.view(batch_size, self.hidden_channel,
                                                     self.num_views * img_feat_stack.shape[-1])

                normal_img_query_pos = normal_img_feats_pos_repeat.gather(
                    index=img_top_proposals_pos_id[:, None, :].permute(0, 2, 1).expand(-1, -1,
                                                                                       normal_img_feats_pos_stack.shape[
                                                                                           -1]), dim=1
                )  # [BS, num_proposals, 2]
                ...
                one_hot = F.one_hot(img_top_proposals_class, num_classes=self.num_classes).permute(0, 2,
                                                                                                   1)  # [BS, num_classes, num_proposals]
                ...
                normal_img_query_pos = self.img_query_pos.repeat(batch_size, 1, 1).to(
                    img_feat.device)  # [BS, num_proposals, 2]
                img_query_pos_view = torch.arange(self.num_img_proposals).reshape(1, -1).repeat(batch_size, 1).to(
                    img_feat.device)
        ...
        if self.with_pts:
            if self.fuse_bev_feature:
                pts_query_feat, pts_query_pos, pts_ret_dicts = self.point_transformer(pts_query_feat, pts_query_pos,
                                                                                      fuse_lidar_feat_flatten, bev_pos)
            else:
                pts_query_feat, pts_query_pos, pts_ret_dicts = self.point_transformer(pts_query_feat, pts_query_pos,
                                                                                      lidar_feat_flatten, bev_pos)
        ...
        if self.with_img:
            ...
            img_query_feat, normal_img_query_pos, img_ret_dicts = self.img_transformer(img_query_feat,
                                                                                       normal_img_query_pos,
                                                                                       img_query_view, img_feats,
        ...
        if self.with_img and self.with_pts:
            ...
            if self.depth_stop_grad:
                img_query_feat, img_query_pos_bev, depth_ret_dicts, extra_mul = self.depth_transformer(
                    pts_query_feat.detach().clone(), pts_query_pos, pts_query_height, img_query_feat.detach().clone(),
                    normal_img_query_pos, img_query_view, normal_img_query_bboxes, img_metas,
                    img_feat_flatten.detach().clone(), normal_img_feat_pos)
            else:
                img_query_feat, img_query_pos_bev, depth_ret_dicts, extra_mul = self.depth_transformer(pts_query_feat,
                                                                                                       pts_query_pos,
                                                                                                       pts_query_height,
                                                                                                       img_query_feat,
                                                                                                       normal_img_query_pos,
                                                                                                       img_query_view,
                                                                                                       normal_img_query_bboxes,
                                                                                                       img_metas,
                                                                                                       img_feat_flatten,
                                                                                                       normal_img_feat_pos)
            ...
            all_query_feat, all_query_pos, fusion_ret_dicts = self.fusion_transformer(pts_query_feat, pts_query_pos,
                                                                                      img_query_feat, img_query_pos_bev)
        ...
        if self.initialize_by_heatmap:
            if self.with_pts:
                ret_dicts[0]['query_heatmap_score'] = heatmap.gather(
                    index=pts_top_proposals_index[:, None, :].expand(-1, self.num_classes, -1),
                    dim=-1)  # [bs, num_classes, num_proposals]
                ret_dicts[0]['dense_heatmap'] = dense_heatmap
            if self.with_img:
                ret_dicts[0]['img_query_heatmap_score'] = img_heatmap.gather(
                    index=img_top_proposals_index[:, None, :].expand(-1, self.num_classes, -1),
                    dim=-1)  # [bs, num_classes, num_proposals]
    ...
    def generate_heatmap_colattn(self, lidar_feat, img_feat, voxel_height, img_metas):
        # img_feat [bs*num_view, C, img_h, img_w]
        # lidar_feat [BS, C, H, W]

        batch_size = lidar_feat.shape[0]
        H, W = lidar_feat.shape[2], lidar_feat.shape[3]
        img_h, img_w = img_feat[0].shape[2], img_feat[0].shape[3]
        ...
        for lvl in range(self.level_num):
            ...
            img_h_lvl, img_w_lvl = img_feat[lvl].shape[-2], img_feat.shape[-1]
            if lvl != 0:
                img_feat[lvl] = F.interpolate(img_feat[lvl], size=(img_h_lvl, img_w), mode='bilinear')
            img_feat[lvl] = img_feat[lvl].reshape(batch_size, self.num_views, self.hidden_channel, img_h_lvl,
                                                  img_w)  # (bs, num_view, C, img_h, img_w)
            level_start_index.append(level_start_index[-1] + img_h_lvl * img_w_lvl)
            spatial_shapes.append([img_h_lvl, img_w_lvl])

            if self.colattn_pos:
                img_pos_lvl = torch.arange(img_h_lvl) + 0.5
                img_pos_lvl = img_pos_lvl.view(1, img_h_lvl, 1).to(lidar_feat.device)  # [1, img_h, 1]
                img_pos_lvl = img_pos_lvl.repeat(img_w, 1, 1)  # [img_w, img_h, 1]
                img_pos.append(img_pos_lvl)
        ...
        for sample_idx in range(batch_size):
            ...
            for view_idx in range(self.num_views):
                ...
                center_ys = center_ys.long().squeeze()  # [num_proposal_on_img, ]

                on_the_image = (center_xs >= 0) & (center_xs < red_img_w) & (center_ys >= 0) & (center_ys < red_img_h) & \
                               valid_height_mask[sample_idx]
                ...
                lidar_feat_view = lidar_feat_flatten[sample_idx, :, on_the_image].transpose(1, 0)  # [N, C]

                img_feat_view = []
                img_pos_view_sine = []
                for lvl in range(self.level_num):
                    img_feat_lvl = img_feat[lvl]
                    img_h_lvl = img_feat[lvl].shape[-2]
                    red_img_h_lvl = int(valid_shape[1] / img_h * img_h_lvl)
                    img_feat_view_lvl = img_feat_lvl[sample_idx, view_idx]  # [C, red_img_h_lvl, red_img_w]
                    img_feat_view.append(img_feat_view_lvl)

                # img_feat_view = img_feat[sample_idx, view_idx, :, :red_img_h, :red_img_w]  # [C, red_img_h, red_img_w]
                img_feat_view = torch.cat(img_feat_view, dim=1)  # [C, img_h_sum, red_img_w]
                img_feat_view = img_feat_view.permute(2, 0, 1)  # [red_img_w, C, img_h_sum]

                if self.colattn_pos:
                    output, weights = self.col_decoder(lidar_feat_view, img_feat_view, lidar_pos_view_sine,
                                                       img_pos_view_sine, need_weights=True)  # [red_img_w, C, max_num]
                else:
                    output, weights = self.col_decoder(lidar_feat_view, img_feat_view, None, None,
                                                       need_weights=True)  # [img_w, C, max_num]
    ...
    #         if lvl != 0:
    #             img_feat[lvl] = F.interpolate(img_feat[lvl], size=(img_h_lvl, img_w), mode='bilinear')
    #         img_feat[lvl] = img_feat[lvl].reshape(batch_size, self.num_views, self.hidden_channel, img_h_lvl, img_w)  # (bs, num_view, C, img_h, img_w)
    #
    #         if self.colattn_pos:
    #             img_pos_lvl = torch.arange(img_h_lvl) + 0.5
    #             img_pos_lvl = img_pos_lvl.view(1, img_h_lvl, 1).to(lidar_feat.device)  # [1, img_h, 1]
    ...
    #                 lidar_pos_view = lidar_pos_view / 75 * 2 * np.pi
    #
    #                 L = torch.arange(128, dtype=torch.float32, device=lidar_pos_view.device)
    #                 dim_t = 10000 ** (2 * (L // 2) / L.shape[0])  # [128, ]
    #                 lidar_pos_view = lidar_pos_view / dim_t  # [red_img_w, max_num, 128]
    #                 lidar_pos_view_sine = torch.stack((lidar_pos_view[..., 0::2].sin(), lidar_pos_view[..., 1::2].cos()), dim=2).view(red_img_w, max_num, 128)
    #
    ...
    #                 img_h_lvl = img_feat[lvl].shape[-2]
    #                 red_img_h_lvl = int(valid_shape[1]/img_h * img_h_lvl)
    #                 img_feat_view_lvl = img_feat_lvl[sample_idx, view_idx, :, :red_img_h_lvl, :red_img_w]  # [C, red_img_h_lvl, red_img_w]
    #                 img_feat_view.append(img_feat_view_lvl)
    #
    #                 if self.colattn_pos:
    #                     img_pos_view_lvl = (img_pos[lvl][:red_img_w, :red_img_h_lvl].clone() / red_img_h_lvl) * 2 * np.pi
    #
    #                     L = torch.arange(128, dtype=torch.float32, device=img_pos_view_lvl.device)
    #                     dim_t = 10000 ** (2 * (L // 2) / L.shape[0])  # [128, ]
    #                     img_pos_view_lvl = img_pos_view_lvl / dim_t  # [red_img_w, red_img_h_lvl, 128]
    #                     img_pos_view_lvl = torch.stack((img_pos_view_lvl[..., 0::2].sin(),
    #                             img_pos_view_lvl[..., 1::2].cos()), dim=2).view(red_img_w, red_img_h_lvl, 128)
    #                     img_pos_view_sine.append(img_pos_view_lvl)
    #
    #             if self.colattn_pos:
    #                 img_pos_view_sine = torch.cat(img_pos_view_sine, dim=1)  # [red_img_w, img_h_sum, 128]
    #
    #             # img_feat_view = img_feat[sample_idx, view_idx, :, :red_img_h, :red_img_w]  # [C, red_img_h, red_img_w]
    #             img_feat_view = torch.cat(img_feat_view, dim=1)  # [C, img_h_sum, red_img_w]
    #             img_feat_view = img_feat_view.permute(2, 0, 1)  # [red_img_w, C, img_h_sum]
    #
    #             if self.colattn_pos:
    #                 output, weights = self.col_decoder(lidar_feat_view, img_feat_view, lidar_pos_view_sine, img_pos_view_sine, need_weights=True) # [red_img_w, C, max_num]
    #             else:
    #                 output, weights = self.col_decoder(lidar_feat_view, img_feat_view, None, None, need_weights=True) # [img_w, C, max_num]
    #             lidar_feat_output[sample_idx, :, on_the_image] = output[center_xs, :, index].clone().transpose(1, 0)
    #
    #     lidar_feat_output = lidar_feat_output.reshape(batch_size, self.hidden_channel, H, W)
    #     if self.cat_point:
    ...
    def generate_heatmap(self, lidar_feat, voxel_height, batch_size, img_metas, img_feat=None):
        ...
        if img_feat is None:
            ...
            dense_heatmap_cross, fuse_lidar_feature_flatten = self.generate_heatmap_colattn(lidar_feat, img_feat,
                                                                                            voxel_height, img_metas)
            if self.cross_only:
        ...
        top_proposals = heatmap.reshape(batch_size, -1).argsort(dim=-1, descending=True)[...,
                        :self.num_proposals]  # [BS, num_proposals]
    ...
    def generate_heatmap_img(self, img_feats, batch_size):
        ...
        for lvl in range(self.level_num):
            ...
            img_heatmap = img_heatmap.view(batch_size, self.num_views, img_heatmap.shape[1],
                                           -1)  # [BS, num_views, num_class, h*w]
            ...
            img_dense_heatmap = img_dense_heatmap.view(batch_size, self.num_views, img_dense_heatmap.shape[1],
                                                       img_dense_heatmap.shape[2],
                                                       img_dense_heatmap.shape[3])  # [BS, num_views, num_class, h, w]
            img_dense_heatmap = img_dense_heatmap.permute(0, 2, 1, 3, 4)  # [BS, num_class, num_views, h, w]
            img_dense_heatmap = img_dense_heatmap.view(batch_size, self.num_classes, self.num_views,
                                                       img_dense_heatmap.shape[-2] * img_dense_heatmap.shape[-1])
            img_dense_heatmaps.append(img_dense_heatmap)

        img_heatmap_stack = torch.cat(img_heatmaps, dim=3)  # [BS, num_class, num_views, h*w (sum)]
        # top #num_proposals among all classes
        top_proposals = img_heatmap_stack.view(batch_size, -1).argsort(dim=-1, descending=True)[...,
                        :self.num_img_proposals]  # [BS, num_proposals]
        top_proposals_class = top_proposals // (
                    img_heatmap_stack.shape[-1] * img_heatmap_stack.shape[-2])  # [BS, num_proposals]

        top_proposals_view_index = top_proposals % (img_heatmap_stack.shape[-1] * img_heatmap_stack.shape[-2]) // \
                                   img_heatmap_stack.shape[-1]  # [BS, num_proposals]
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, preds_dict, img_metas):
        ...
        if self.with_img:
            res_tuple_2d = multi_apply(self.get_targets_single_2d, gt_bboxes, gt_labels, gt_img_centers_2d,
                                       list_of_pred_dict, img_metas, np.arange(len(gt_bboxes)))
    ...
    def get_targets_single_2d(self, gt_bboxes, gt_labels, gt_centers_2d, preds_dict, img_metas, batch_idx):
        ...
        for idx_layer in range(self.num_img_decoder_layers):
            bboxes_tensor_layer = bbox[idx_layer * self.num_img_proposals:(idx_layer + 1) * self.num_img_proposals,
                                  :]  # [num_proposals, 4]
            score_layer = cls[idx_layer * self.num_img_proposals:(idx_layer + 1) * self.num_img_proposals,
                          :]  # [num_proposals, num_class]
            view_layer = view[
                         idx_layer * self.num_img_proposals:(idx_layer + 1) * self.num_img_proposals]  # [num_proposals]
            centers_layer = centers[idx_layer * self.num_img_proposals:(idx_layer + 1) * self.num_img_proposals,
                            :]  # [num_proposals, 2]
            offsets_layer = offsets[idx_layer * self.num_img_proposals:(idx_layer + 1) * self.num_img_proposals, :]

            assign_result = self.bbox_assigner_2d.assign(bboxes_tensor_layer, score_layer, centers_layer, offsets_layer,
                                                         view_layer, gt_bboxes, gt_labels, gt_centers_2d[..., :2],
                                                         gt_offsets, img_w=img_shape[1], img_h=img_shape[0])
        ...
        if len(pos_inds) > 0:
            ...
            if self.depth_reg:
                ...
                depth = torch.clamp(gt_centers_2d[sampling_result.pos_assigned_gt_inds, 2], self.dbound[0],
                                    self.dbound[1])
        ...
        if self.initialize_by_heatmap:
            ...
            heatmaps = []
            for lvl in range(self.level_num):
                heatmaps.append(gt_bboxes.new_zeros(self.num_classes, self.num_views, h, w))
                h = h // 2
                w = w // 2
            ...
            for lvl in range(self.level_num):
                heatmaps[lvl] = heatmaps[lvl].view(self.num_classes, self.num_views,
                                                   heatmaps[lvl].shape[-2] * heatmaps[lvl].shape[-1])
            heatmap = torch.cat(heatmaps, dim=-1)
            matched_ious = torch.ones_like(ious) * -1
            matched_ious[pos_inds] = ious[pos_inds]

            return labels[None], label_weights[None], bbox_targets[None], bbox_weights[None], center_targets[None], \
                   center_weights[None], depth_labels[None], depth_weights[None], ious[None], pos_num_layers[None], \
                   matched_ious[None], heatmap[None]
        else:
            matched_ious = torch.ones_like(ious) * -1
            matched_ious[pos_inds] = ious[pos_inds]
            return labels[None], label_weights[None], bbox_targets[None], bbox_weights[None], center_targets[None], \
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict, batch_idx):
        ...
        for idx_layer in range(num_layer):
            ...
            if self.train_cfg.assigner.type == 'HungarianAssigner3D':
                assign_result = self.bbox_assigner.assign(bboxes_tensor_layer, gt_bboxes_tensor, gt_labels_3d,
                                                          score_layer, self.train_cfg)
            elif self.train_cfg.assigner.type == 'HeuristicAssigner':
                assign_result = self.bbox_assigner.assign(bboxes_tensor_layer, gt_bboxes_tensor, None, gt_labels_3d,
        ...
        if self.initialize_by_heatmap:
            ...
            matched_ious = torch.ones_like(ious) * -1
            matched_ious[pos_inds] = ious[pos_inds]
            return labels[None], label_weights[None], bbox_targets[None], bbox_weights[None], ious[None], \
                   pos_num_layers[None], matched_ious[None], heatmap[None]
        else:
            matched_ious = torch.ones_like(ious) * -1
            matched_ious[pos_inds] = ious[pos_inds]
            return labels[None], label_weights[None], bbox_targets[None], bbox_weights[None], ious[None], \
                   pos_num_layers[None], matched_ious[None]

    @force_fp32(apply_to=('preds_dicts'))
    def loss(self, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_pts_centers_2d, gt_img_centers_2d, img_metas,
             preds_dicts, **kwargs):
        ...
        if self.with_img:
            if self.initialize_by_heatmap:
                ...
                labels_2d, label_weights_2d, bbox_targets_2d, bbox_weights_2d, center_targets_2d, center_weights_2d, \
                depth_2d, depth_weight_2d, ious_2d, num_pos_layer_2d, matched_ious_2d, heatmap_2d = self.get_targets(
                    gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, preds_dicts[0], img_metas)
            else:
                labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious, \
                labels_2d, label_weights_2d, bbox_targets_2d, bbox_weights_2d, center_targets_2d, center_weights_2d, \
                depth_2d, depth_weight_2d, ious_2d, num_pos_layer_2d, matched_ious_2d = self.get_targets(gt_bboxes_3d,
                                                                                                         gt_labels_3d,
                                                                                                         gt_bboxes,
                                                                                                         gt_labels,
                                                                                                         gt_img_centers_2d,
                                                                                                         preds_dicts[0],
                                                                                                         img_metas)  # if hasattr(self, 'on_the_image_mask'):
        else:
            if self.initialize_by_heatmap:
                labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious, heatmap = self.get_targets(
                    gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, preds_dicts[0], img_metas)
            else:
                labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious = self.get_targets(
                    gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, preds_dicts[0],
                    img_metas)  # if hasattr(self, 'on_the_image_mask'):
        ...
        if self.initialize_by_heatmap:
            ...
            loss_heatmap = self.loss_heatmap(clip_sigmoid(preds_dict['dense_heatmap']), heatmap,
            ...
            if 'valid_shape' in img_metas[0].keys():
                ...
                for lvl in range(self.level_num):
                    heatmap_2d_weight = torch.zeros(heatmap_2d.shape[0], self.num_classes, self.num_views, img_h,
                                                    img_w).to(heatmap_2d.device)
                ...
                for lvl in range(self.level_num):
                    heatmaps_2d_weight[lvl] = heatmaps_2d_weight[lvl].view(heatmaps_2d_weight[lvl].shape[0],
                                                                           self.num_classes, self.num_views,
                                                                           heatmaps_2d_weight[lvl].shape[-2] *
                                                                           heatmaps_2d_weight[lvl].shape[-1])
                ...
                loss_heatmap_2d = self.loss_heatmap_2d(clip_sigmoid(preds_dict['img_dense_heatmap']), heatmap_2d,
                                                       weight=heatmap_2d_weight,
                                                       avg_factor=max(heatmap_2d.eq(1).float().sum().item(), 1))
        ...
        for idx_layer in range(self.num_pts_decoder_layers + self.num_fusion_decoder_layers):
            ...
            layer_loss_cls = self.loss_cls(layer_cls_score, layer_labels, layer_label_weights,
                                           avg_factor=max(num_pos_layer[idx_layer], 1))
            ...
            preds = torch.cat([layer_center, layer_height, layer_dim, layer_rot], dim=1).permute(0, 2,
                                                                                                 1)  # [BS, num_proposals, code_size]
            if 'vel' in preds_dict.keys():
                layer_vel = preds_dict['vel'][..., start:start + layer_num_proposals]
                preds = torch.cat([layer_center, layer_height, layer_dim, layer_rot, layer_vel], dim=1).permute(0, 2,
                                                                                                                1)  # [BS, num_proposals, code_size]
            ...
            layer_loss_bbox = self.loss_bbox(preds, layer_bbox_targets, layer_reg_weights,
                                             avg_factor=max(num_pos_layer[idx_layer], 1))

            layer_match_ious = matched_ious[..., start:start + layer_num_proposals]
            layer_match_ious = torch.sum(layer_match_ious * (layer_match_ious >= 0), dim=-1) / torch.sum(
                layer_match_ious >= 0, dim=-1)
        ...
        if self.with_img and self.supervision2d:
            ...
            for idx_layer in range(self.num_img_decoder_layers):
                ...
                layer_loss_cls_2d = self.loss_cls_2d(layer_cls_score_2d, layer_labels_2d, layer_label_weights_2d,
                                                     avg_factor=max(num_pos_layer_2d[idx_layer], 1))

                preds_2d_center = preds_dict['center_2d'][..., start:start + layer_num_proposals].permute(0, 2,
                                                                                                          1)  # [bs, num_proposal, 4]
                preds_2d_offset = preds_dict['offset'][..., start:start + layer_num_proposals].permute(0, 2,
                                                                                                       1)  # [bs, num_proposal, 4]
                ...
                layer_loss_bbox_2d = self.loss_bbox_2d(preds_2d, layer_bbox_targets_2d, layer_reg_weights_2d,
                                                       avg_factor=max(num_pos_layer_2d[idx_layer], 1))

                layer_match_ious_2d = matched_ious_2d[..., start:start + layer_num_proposals]
                layer_match_ious_2d = torch.sum(layer_match_ious_2d * (layer_match_ious_2d >= 0), dim=-1) / (
                            torch.sum(layer_match_ious_2d >= 0, dim=-1) + 1e-2)
                ...
                if idx_layer == self.num_img_decoder_layers - 1:
                    ...
                    loss_center_2d = self.loss_center_2d(pred_centers, center_targets_2d, center_weights_2d,
                                                         avg_factor=max(num_pos_layer_2d[idx_layer], 1))
                    ...
                    if self.depth_reg:
                        ...
                        loss_depth = self.loss_depth_reg(pred_depth, depth_labels, depth_weights,
                                                         avg_factor=max(num_pos_layer_2d[idx_layer], 1))
                        ...
                        loss_depth = self.loss_depth(pred_depth, depth_labels, depth_weights,
                                                     avg_factor=max(num_pos_layer_2d[idx_layer], 1))
                        loss_dict[f'{prefix}_loss_depth_cls'] = loss_depth

                    if self.extra_class_sup:
                        depth_layer_cls_score_2d = preds_dict['extra_cls'].permute(0, 2, 1).reshape(-1,
                                                                                                    self.num_classes)
                        depth_layer_loss_cls_2d = self.loss_cls_2d(depth_layer_cls_score_2d, layer_labels_2d,
                                                                   layer_label_weights_2d,
                                                                   avg_factor=max(num_pos_layer_2d[idx_layer], 1))
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False, for_roi=False):
        ...
        for id, preds_dict in enumerate(preds_dicts):
            layer_num_proposal = self.get_layer_num_proposal(
                self.num_pts_decoder_layers + self.num_fusion_decoder_layers - 1)
