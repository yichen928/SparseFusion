...
class ImplicitHead2D_3D_CrossProj(nn.Module):
        ...
        if self.with_img:
            ...
            if self.initialize_by_heatmap:
                ...
                img_heatmap, img_dense_heatmap, img_top_proposals_class, img_top_proposals_index, img_top_proposals_view_idx, img_top_proposals_pos_id = \
                    self.generate_heatmap_img(img_feat_heatmap, batch_size, img_metas)
    ...
    def generate_heatmap_img(self, img_feat, batch_size, img_metas):
        ...
        if batch_size > 1:  # training stage
            for sample_idx in range(batch_size):
                for view_idx in range(self.num_views):
                    valid_w, valid_h = img_metas[sample_idx]['valid_shape'][view_idx]
                    valid_w = int(valid_w / self.out_size_factor_img)
                    valid_h = int(valid_h / self.out_size_factor_img)
                    valid_mask = torch.zeros([1, img_heatmap.shape[-2], img_heatmap.shape[-1]]).type_as(img_heatmap)
                    valid_mask[:, :valid_h, :valid_w] = 1
                    img_heatmap[sample_idx * self.num_views + view_idx] = img_heatmap[sample_idx * self.num_views + view_idx] * valid_mask
    ...
    def get_targets_single_2d(self, gt_bboxes, gt_labels, gt_centers_2d, preds_dict, img_metas, batch_idx):
        ...
        img_shape = img_metas['input_shape']
