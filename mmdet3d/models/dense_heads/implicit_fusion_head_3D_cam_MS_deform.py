...
from mmdet3d.models.utils import FFN, TransformerDecoderLayer, PositionEmbeddingLearned, PositionEmbeddingLearnedwoNorm,\
    PointTransformer2D_3D, FusionTransformer2D_3D_Cross, DepthTransformer2D_3D, PointProjection, ImageProjection, \
...
class ImplicitHead2D_Cam_MS_Deform(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
                 pts_smca=False,
                 ...
                 loss_depth_laplace=dict(type='LaplaceL1Loss', reduction='mean'),
                 # others
                 train_cfg=None,
                 test_cfg=None,
                 bbox_coder=None,
                 ...
                 view_transform_point_proj=True,
                 view_transform_rematch=True,
                 view_transform_pos_early=False,
                 view_bbox_add=False,
                 bbox_pos=False,
                 # heatmap_query_level='max', # deleted in Feb. 19
                 query_dropout=0.0,
                 double_gt=False,
                 fusion_gate=False,
                 fusion_cwa=False,
        ...
        self.view_transform_point = view_transform_point
        self.view_transform_FFN = view_transform_FFN
        self.view_transform_point_proj = view_transform_point_proj
        self.view_transform_rematch = view_transform_rematch
        ...
        self.loss_depth_laplace = build_loss(loss_depth_laplace)
        ...
        self.depth_for_proposal = depth_for_proposal
        ...
        self.img_heatmap_LN = img_heatmap_LN

        self.double_gt = double_gt
        self.fusion_gate = fusion_gate
        self.fusion_cwa = fusion_cwa
        self.ignorance_loss = ignorance_loss
        ...
        if with_img:
            ...
            if view_transform:
                ...
                if self.bbox_pos:  
                    ...
                    if self.view_transform_point:
                        view_query_pos_embed = PositionEmbeddingLearned(2, hidden_channel)
                        view_key_pos_embed = PositionEmbeddingLearned(2, hidden_channel)
                    ...
                    if self.view_transform_point:
                        ...
                        view_query_pos_embed = PositionEmbeddingLearned(3, hidden_channel)
                    else:
                        # view_query_pos_embed = PositionEmbeddingLearnedLN(9, hidden_channel)
                        # view_key_pos_embed = PositionEmbeddingLearnedLN(9, hidden_channel)

                        view_query_pos_embed = PositionEmbeddingLearned(9, hidden_channel)
                        view_key_pos_embed = PositionEmbeddingLearned(9, hidden_channel)

                        # view_query_pos_embed = PositionEmbeddingLearnedwoNorm(3, hidden_channel)
                        # view_key_pos_embed = view_query_pos_embed

                view_projection = ProjectionLayerNorm(hidden_channel)

                if self.view_transform_point:
                    if self.view_transform_point_proj:
                        view_bev_projection = ConvLN(hidden_channel, hidden_channel, kernel_size=3, stride=1, padding=1, require_act=False)
                    else:
                        view_bev_projection = nn.Identity()
                    self.view_transformer = ViewTransformerPoint(
                        ...
                        bbox_pos=bbox_pos, bev_projection=view_bev_projection
                    )
                elif self.view_transform_FFN:
                    self.view_transformer = ViewTransformerFFN(
                        hidden_channel=hidden_channel, num_heads=num_heads, prediction_heads=view_prediction_heads,
                        ffn_channel=ffn_channel, dropout=dropout, activation=activation, test_cfg=test_cfg,
                        query_pos=view_query_pos_embed, view_projection=view_projection
                    )
                elif self.view_bbox_add:
                    # heads_view = dict(center_view=(2, 2, reg_bn), height_view=(1, 2, reg_bn), dim_view=(3, 2, reg_bn),
                    #                   rot_view=(2, 2, reg_bn), vel_view=(2, 2, reg_bn), heatmap_view=(self.num_classes, 2))
                    # view_prediction_heads = FFN(hidden_channel, heads_view, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias)
                    # view_projection = ProjectionLayerNorm(hidden_channel)
                    self.view_adder = ViewAdder(hidden_channel=hidden_channel, ffn_channel=ffn_channel, dropout=dropout,
                        test_cfg=test_cfg, view_projection=view_projection, prediction_heads=view_prediction_heads,
                    )
                else:
                    self.view_transformer = ViewTransformer(
                        ...
                        ffn_channel=ffn_channel, dropout=dropout, activation=activation, test_cfg=test_cfg,
                        query_pos=view_query_pos_embed, key_pos=view_key_pos_embed, view_projection=view_projection,
                        bbox_pos=bbox_pos, use_camera=use_camera, pos_early=view_transform_pos_early, virtual_depth=virtual_depth
                    ) 
        ...
        if with_pts and with_img:
            ...
            if fuse_self:
                ...
                # fusion_query_pos_embed = [PositionEmbeddingLearned(2, hidden_channel) for _ in range(self.num_fusion_decoder_layers*2)]
                # fusion_key_pos_embed = [PositionEmbeddingLearned(2, hidden_channel) for _ in range(self.num_fusion_decoder_layers*2)]
                # self.fusion_transformer = FusionTransformer2D_3D_DoubleCross(
                #     hidden_channel=hidden_channel, num_heads=num_heads, num_decoder_layers=num_fusion_decoder_layers,
                #     prediction_heads=fusion_prediction_heads,  ffn_channel=ffn_channel, dropout=dropout,
                ...
                # )
                # self.fusion_transformer = FusionTransformer2D_3D_SepPos_Self(
                #     hidden_channel=hidden_channel, num_heads=num_heads, num_decoder_layers=num_fusion_decoder_layers,
                #     prediction_heads=fusion_prediction_heads,  ffn_channel=ffn_channel, dropout=dropout,
                #     activation=activation, test_cfg=test_cfg, pts_pos=fusion_query_pos_embed, img_pos=fusion_key_pos_embed,
                #     pts_projection=fuse_pts_projection, img_projection=fuse_img_projection, num_proposals=num_proposals
                # )
            else:
            ...
            if self.initialize_by_heatmap and self.cross_heatmap:
                ...
                if self.cross_type == 'deform_height':
                    ...
                    # colattn_query_pos = PositionEmbeddingLearned(1, hidden_channel)
                    # colattn_key_pos = PositionEmbeddingLearned(3, hidden_channel)
        ...
        if self.with_img:
            ...
            if self.encode_depth:
                if depth_encoder == 'large':
                    self.shared_conv_img = DepthEncoderLarge(in_channels_img+2, hidden_channel)
                elif depth_encoder == 'small':
                    self.shared_conv_img = nn.ModuleList()
                    for i in range(self.level_num):
                        self.shared_conv_img.append(DepthEncoderSmall(in_channels_img+2, hidden_channel, depth_encoder_dcn))
                    ...
                    if self.depth_for_proposal:
                        ...
                        self.depth_resnet = DepthEncoderResNet(depth_input_channel, in_channels_img, hidden_channel, depth_layers=blocks, proj_input=True)
                elif depth_encoder == 'resnet_simple':
                    self.shared_conv_img = nn.Identity()
                    if self.depth_for_proposal:
                        self.depth_resnet = DepthEncoderResNetSimple(depth_input_channel, in_channels_img, hidden_channel)
                    else:
                        self.depth_resnet = DepthEncoderResNetSimple(depth_input_channel, in_channels_img, hidden_channel, proj_input=True)
                # else:
                #     self.shared_conv_img = DepthEncoder(in_channels_img+2, hidden_channel)
    ...
    def init_weights(self):
        ...
        if self.sep_level_pos:
            level_pos_heatmap = torch.zeros([self.level_num, self.hidden_channel])
            level_pos_img = torch.zeros([self.level_num, self.hidden_channel])
            self.level_pos_heatmap = nn.Parameter(level_pos_heatmap, requires_grad=True)
            torch.nn.init.normal_(self.level_pos_heatmap)
            self.level_pos_img = nn.Parameter(level_pos_img, requires_grad=True)
            torch.nn.init.normal_(self.level_pos_img)
        else:
            if 'resnet' in self.depth_encoder and self.encode_depth:
    ...
    def forward_single(self, inputs, img_inputs, img_metas, sparse_depth):
        ...
        if self.encode_depth:
            ...
            if 'resnet' in self.depth_encoder:
                if self.depth_for_proposal:
        ...
        if self.with_img:
            for i in range(self.level_num):
                if self.sep_level_pos:
                    img_inputs_level = img_inputs[i]
                else:
                    img_inputs_level = img_inputs[i] + self.level_pos[i].reshape(1, self.level_pos[i].shape[0], 1, 1)
                if self.encode_depth and 'resnet' not in self.depth_encoder:
                    h_level = img_inputs_level.shape[-2]
                    w_level = img_inputs_level.shape[-1]
                    sparse_depth_level = sparse_depth[:, i, :, :h_level, :w_level]
                    img_inputs_level = torch.cat([img_inputs_level, sparse_depth_level], dim=1)
                if self.depth_encoder == 'small':
                    img_feat = self.shared_conv_img[i](img_inputs_level)
                else:
        ...
        if self.with_pts:
            ...
            if self.initialize_by_heatmap:
                if self.with_img and self.cross_heatmap:
                    ...
                    if self.sep_level_pos:
                        for lvl in range(self.level_num):
                            img_feat_cross[lvl] = img_feat_cross[lvl] + self.level_pos_heatmap[lvl].reshape(1, self.level_pos_heatmap[lvl].shape[0], 1, 1)
                else:
                    img_feat_cross = None

                heatmap, dense_heatmap, pts_top_proposals_class, pts_top_proposals_index, fuse_lidar_feat_flatten = self.generate_heatmap(lidar_feat.clone(), min_voxel_height, max_voxel_height, batch_size, img_metas, proj_matrix['lidar2img_rt'], img_feat_cross, input_padding_mask)
        ...
        if self.with_img:
            if self.initialize_by_heatmap:
                ...
                if self.depth_for_proposal:
                    ...
                    for lvl in range(self.level_num):
                        img_feats_heatmap.append(img_inputs_wodepth[lvl].clone())

                if self.img_heatmap_stop_grad:
                    for lvl in range(self.level_num):
                        img_feats_heatmap[lvl] = img_feats_heatmap[lvl].detach()
        ...
        # torch.save(pts_query_pos, "vis/pts_query_pos_heatmap.pt")
        ...
        if self.with_pts:
            ...
            # torch.save(pts_top_proposals_class, "vis/pts_query_label.pt")
        ...
        if self.with_img:
            ...
            # torch.save(img_ret_dicts[-1]['cls'], "vis/img_query_cls.pt")

            if self.view_transform:
                if self.view_transform_point:
                    img_query_feat, img_query_pos_bev, view_ret_dicts = self.view_transformer(img_query_feat, img_query_pos_bev, lidar_feat_flatten, bev_pos, img_ret_dicts, camera_info)
                elif self.view_bbox_add:
        ...
        if self.with_img and self.with_pts:
            if self.query_dropout > 0 and pts_query_feat.shape[0] > 0:
                if np.random.random() < self.query_dropout:
                    ids = np.arange(self.num_proposals)
        ...
        # torch.save(fusion_ret_dicts[-1]['heatmap'], 'vis/fusion_score.pt')
    ...
    #     if self.virtual_depth:
    #         if 'pcd_scale_factor' in img_metas[0]:
    #             for sample_idx in range(batch_size):
    #                 depth[sample_idx] = depth[sample_idx] / img_metas[sample_idx]['pcd_scale_factor']
    #         if 'img_scale_ratios' in img_metas[0]:
    #             for sample_idx in range(batch_size):
    #                 for view_idx in range(self.num_views):
    #                     depth[sample_idx, view_idx] = depth[sample_idx, view_idx] / img_metas[sample_idx]['img_scale_ratios'][view_idx]
    ...
    #         lidar_feat_count = lidar_feat_count.reshape(batch_size, 1, H, W)
    #         lidar_feat_flag = torch.where(lidar_feat_count>0, torch.ones_like(lidar_feat_count), torch.zeros_like(lidar_feat_count))
    #         lidar_feat_output = torch.cat([lidar_feat_output, lidar_feat, lidar_feat_flag], dim=1)
    #         lidar_feat_output = self.reduce_conv(lidar_feat_output)
    #         # lidar_feat_output = self.se_block(lidar_feat_output)
    #     else:
    #         # lidar_feat_output = self.reduce_conv(lidar_feat_output)
    ...
    def generate_heatmap_deform(self, lidar_feat, img_feat, voxel_height, img_metas, lidar2img_rt, input_padding_mask):
        ...
        for sample_idx in range(batch_size):
            ...
            for view_idx in range(self.num_views):
                ...
                pts_2d_view = points_2d[sample_idx, view_idx]
                img_coors = pts_2d_view[:, 0:2]

                # grid sample, the valid grid range should be in [-1,1]
                center_xs, center_ys = torch.split(img_coors, 1, dim=1)  # each is Nx1
                ...
                centers_normal = reference_points * 2 - 1
                centers_normal = centers_normal[None, None]

                for lvl in range(self.level_num):
                    img_feat_lvl = F.grid_sample(img_feat[lvl][sample_idx:sample_idx + 1, view_idx], centers_normal,
                                                 mode='bilinear', padding_mode="border", align_corners=False)
                    img_feat_lvl = img_feat_lvl[0, :, 0]
                ...
                nonoverlap_mask = torch.logical_not(overlap_mask)
                lidar_feat_output_view = lidar_feat_output[sample_idx, :, on_the_image]
                lidar_feat_output_view[:, overlap_mask] = torch.maximum(lidar_feat_output_view[:, overlap_mask], output[0, :, overlap_mask])
                lidar_feat_output_view[:, nonoverlap_mask] = output[0, :, nonoverlap_mask]
                lidar_feat_output[sample_idx, :, on_the_image] = lidar_feat_output_view
                lidar_feat_count[sample_idx, :, on_the_image] += 1
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, gt_bboxes_cam_3d, gt_visible, gt_bboxes_lidar, preds_dict, img_metas):
        ...
        if self.with_img:
            res_tuple_2d = multi_apply(self.get_targets_single_2d, gt_bboxes, gt_labels, gt_img_centers_2d, gt_bboxes_cam_3d, gt_bboxes_lidar, list_of_pred_dict, img_metas, np.arange(len(gt_bboxes)))
            ...
            if self.view_transform:
                if self.view_transform_rematch:
                    ...
                    labels_view = torch.cat(res_tuple_2d[-5], dim=0)
                    label_weights_view = torch.cat(res_tuple_2d[-4], dim=0)
                    bbox_targets_view = torch.cat(res_tuple_2d[-3], dim=0)
                    bbox_weights_view = torch.cat(res_tuple_2d[-2], dim=0)
                    num_pos_layer_view = num_pos_layer_2d
                    matched_ious_view = torch.cat(res_tuple_2d[-1], dim=0)
                    ious_view = None
    ...
    def get_targets_single_2d(self, gt_bboxes, gt_labels, gt_centers_2d, gt_bboxes_cam_3d, gt_bboxes_lidar_3d, preds_dict, img_metas, batch_idx):
        ...
        if self.allocentric:
            center_2d = copy.deepcopy(preds_dict['center_2d'].detach())
            cam_intrinsic = torch.Tensor(img_metas['cam_intrinsic']).type_as(score)
            cam2img = cam_intrinsic[view]
            height, width = img_metas['input_shape']
            bboxes_3d_tensor[:, 6] = torch.atan2(center_2d[0, 0] * width - cam2img[:, 0, 2], cam2img[:, 0, 0]) + bboxes_3d_tensor[:, 6]

        if not self.view_transform_rematch:
            # get pred boxes, carefully ! donot change the network outputs
            score_view = copy.deepcopy(preds_dict['heatmap_view'].detach())
            center_view = copy.deepcopy(preds_dict['center_view'].detach())
            height_view = copy.deepcopy(preds_dict['height_view'].detach())
            dim_view = copy.deepcopy(preds_dict['dim_view'].detach())
            rot_view = copy.deepcopy(preds_dict['rot_view'].detach())
            if 'vel_view' in preds_dict.keys():
                vel_view = copy.deepcopy(preds_dict['vel_view'].detach())
            else:
                vel_view = None

            boxes_dict_view = self.bbox_coder.decode(score_view, rot_view, dim_view, center_view, height_view, vel_view)  # decode the prediction to real world metric bbox
            bboxes_tensor_view = boxes_dict_view[0]['bboxes']

        gt_bboxes_3d_tensor = gt_bboxes_cam_3d.tensor.to(score.device)
        gt_bboxes_lidar_3d_tensor = gt_bboxes_lidar_3d.tensor.to(score.device)

        assert gt_bboxes_lidar_3d_tensor.shape[0] == gt_bboxes_3d_tensor.shape[0]
        ...
        # torch.save(view, 'vis/img_view_%d.pt'%batch_idx)
        # torch.save(bboxes_3d_tensor, 'vis/img_bboxes_3d_tensor_%d.pt'%batch_idx)
        ...
        if self.detection_2d:
            bbox_targets = torch.zeros([num_proposals, self.bbox_2d_coder.code_size+4]).to(score.device)
            bbox_weights = torch.zeros([num_proposals, self.bbox_2d_coder.code_size+4]).to(score.device)
        else:
        ...
        if len(pos_inds) > 0:
            ...
            if self.allocentric:
                pos_angle_target = pos_gt_bboxes[:, 6]
                pos_center_2d = gt_centers_2d[sampling_result.pos_assigned_gt_inds, :2]
                pos_view_label = gt_labels[sampling_result.pos_assigned_gt_inds, 1]
                pos_cam2img = cam_intrinsic[pos_view_label]
                pos_angle_target = pos_angle_target - torch.atan2(pos_center_2d[:, 0] - pos_cam2img[:, 0, 2], pos_cam2img[:, 0, 0])
                pos_gt_bboxes[:, 6] = pos_angle_target

            pos_bbox_targets = self.bbox_2d_coder.encode(pos_gt_bboxes)
            bbox_targets[pos_inds, :pos_bbox_targets.shape[1]] = pos_bbox_targets

            if self.detection_2d:
                bbox_targets[pos_inds, -4:] = normal_gt_bboxes[sampling_result.pos_assigned_gt_inds]
            ...
            if not self.view_transform_rematch:
                bbox_lidar_targets[pos_inds_lastlayer, :] = self.bbox_coder.encode(gt_bboxes_lidar_3d_tensor[pos_assigned_gt_inds_lastlayer])
                bbox_lidar_weights[pos_inds_lastlayer, :] = 1.0

                if gt_labels is None:
                    labels_lidar[pos_inds_lastlayer] = 1
                else:
                    labels_lidar[pos_inds_lastlayer] = gt_labels[pos_assigned_gt_inds_lastlayer, 0]

                if self.train_cfg.pos_weight <= 0:
                    label_lidar_weights[pos_inds_lastlayer] = 1.0
                else:
                    label_lidar_weights[pos_inds_lastlayer] = self.train_cfg.pos_weight

                pos_bboxes_lidar = bboxes_tensor_view[pos_inds_lastlayer]
                gt_bboxes_lidar = gt_bboxes_lidar_3d_tensor[pos_assigned_gt_inds_lastlayer]

                bbox_overlap = BboxOverlaps3D(coordinate='lidar')
                pos_ious_lidar = bbox_overlap(pos_bboxes_lidar, gt_bboxes_lidar)
                pos_ious_lidar = torch.diag(pos_ious_lidar)

                ious_lidar[pos_inds_lastlayer] = pos_ious_lidar
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, gt_visible, preds_dict, batch_idx):
        ...
        if self.auxliary_loss:
            num_fusion_decoder_layers = self.num_fusion_decoder_layers
        else:
            num_fusion_decoder_layers = 1
        if self.double_gt:
            num_layer = self.num_pts_decoder_layers + num_fusion_decoder_layers * 2
        else:
        ...
        for idx_layer in range(num_layer):
            ...
            if self.double_gt and idx_layer >= self.num_pts_decoder_layers and (idx_layer - self.num_pts_decoder_layers) % 2 == 1:
                gt_visible_mask = gt_visible == 1
                gt_bboxes_tensor_layer = gt_bboxes_tensor[gt_visible_mask]
                gt_labels_3d_layer = gt_labels_3d[gt_visible_mask]
            else:
                gt_bboxes_tensor_layer = gt_bboxes_tensor
                gt_labels_3d_layer = gt_labels_3d

            if self.ignorance_loss and idx_layer >= self.num_pts_decoder_layers:
                gt_visible_mask = gt_visible == 1
                gt_bboxes_tensor_layer_double = torch.cat([gt_bboxes_tensor,gt_bboxes_tensor[gt_visible_mask]], dim=0)
                gt_labels_3d_layer_double = torch.cat([gt_labels_3d,gt_labels_3d[gt_visible_mask]], dim=0)
            ...
            if self.train_cfg.assigner.type == 'HungarianAssigner3D':
                ...
                if self.ignorance_loss and idx_layer >= self.num_pts_decoder_layers:
                    assign_result_double = self.bbox_assigner.assign(bboxes_tensor_layer, gt_bboxes_tensor_layer_double, gt_labels_3d_layer_double, score_layer, self.train_cfg)
                ...
                if self.ignorance_loss and idx_layer >= self.num_pts_decoder_layers:
                    assign_result_double = self.bbox_assigner.assign(bboxes_tensor_layer, gt_bboxes_tensor_layer_double, None, gt_labels_3d_layer_double, self.query_labels[batch_idx])
            else:
                raise NotImplementedError
            # assign_result_list.append(assign_result)
            ...
            if self.ignorance_loss:
                if idx_layer >= self.num_pts_decoder_layers:
                    sampling_result_layer_double = self.bbox_sampler.sample(assign_result_double, bboxes_tensor_layer, gt_bboxes_tensor_layer_double)
                    pos_inds_layer_double = sampling_result_layer_double.pos_inds + start
                    neg_inds_layer_double = sampling_result_layer_double.neg_inds + start
                else:
                    pos_inds_layer_double = pos_inds_layer
                    neg_inds_layer_double = neg_inds_layer
                neg_inds_double.append(neg_inds_layer_double)
        ...
        if self.ignorance_loss:
            neg_inds_double = torch.cat(neg_inds_double, dim=0)
        ...
        if len(neg_inds) > 0:
            if self.ignorance_loss:
                label_weights[neg_inds_double] = 1.0
            else:
                label_weights[neg_inds] = 1.0
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_pts_centers_2d, gt_img_centers_2d, gt_bboxes_cam_3d, gt_visible_3d, gt_bboxes_lidar, img_metas, preds_dicts, **kwargs):
        ...
        if self.with_img:
            if self.initialize_by_heatmap:
                if self.view_transform:
                    ...
                    labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious, heatmap, \
                    labels_2d, label_weights_2d, bbox_targets_2d, bbox_weights_2d, ious_2d, num_pos_layer_2d, \
                    matched_ious_2d, heatmap_2d = self.get_targets(gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, gt_bboxes_cam_3d, gt_visible_3d, gt_bboxes_lidar, preds_dicts[0], img_metas)
            else:
                if self.view_transform:
                    ...
                    labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious, \
                    labels_2d, label_weights_2d, bbox_targets_2d, bbox_weights_2d, ious_2d, num_pos_layer_2d, matched_ious_2d = \
                        self.get_targets(gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, gt_bboxes_cam_3d, gt_visible_3d, preds_dicts[0], img_metas)        # if hasattr(self, 'on_the_image_mask'):
        else:
            if self.initialize_by_heatmap:
        ...
        if self.with_img:
            ...
            for idx_layer in range(self.num_img_decoder_layers):
                ...
                if self.detection_2d:
                    layer_bbox_targets_img = bbox_targets_2d[:, start:start + layer_num_proposals, -4:]
                    layer_reg_weights_img = bbox_weights_2d[:, start:start + layer_num_proposals, -4:]
                    preds_img_center = preds_dict['center_img'][..., start:start + layer_num_proposals]  # [bs, 2, num_proposal]
                    preds_img_dim = preds_dict['dim_img'][..., start:start + layer_num_proposals]  # [bs, 2, num_proposal]
                    preds_img = torch.cat([preds_img_center, preds_img_dim], dim=1).permute(0, 2, 1)  # [bs, num_proposal, 4]
                    layer_loss_img = self.loss_center_2d(preds_img, layer_bbox_targets_img, layer_reg_weights_img, avg_factor=max(num_pos_layer_2d[idx_layer], 1)) / 5 
                    loss_dict[f'{prefix}_loss_bbox_img'] = layer_loss_img
                ...
                if self.laplace_loss:
                    pred_log_var = preds_2d_depth.permute(0, 2, 1)[..., 1:]
                    pred_depth = torch.cat([preds_2d[...,2:3], pred_log_var], dim=-1)
                    layer_loss_depth_2d = self.loss_depth_laplace(pred_depth, layer_bbox_targets_2d[...,2:3], layer_reg_weights_2d[...,2:3], avg_factor=max(num_pos_layer_2d[idx_layer], 1))

                else:
                    layer_loss_depth_2d = self.loss_depth_reg(preds_2d[...,2:3], layer_bbox_targets_2d[...,2:3], layer_reg_weights_2d[...,2:3], avg_factor=max(num_pos_layer_2d[idx_layer], 1))

                layer_loss_dim_2d = self.loss_bbox_2d(preds_2d[...,3:6], layer_bbox_targets_2d[...,3:6], layer_reg_weights_2d[...,3:6], avg_factor=max(num_pos_layer_2d[idx_layer], 1))
                layer_loss_rot_2d = self.loss_bbox_2d(preds_2d[...,6:8], layer_bbox_targets_2d[...,6:8], layer_reg_weights_2d[...,6:8], avg_factor=max(num_pos_layer_2d[idx_layer], 1))
                ...
                if preds_2d.shape[-1] > 8:
                    layer_loss_vel_2d = self.loss_bbox_2d(preds_2d[...,8:10], layer_bbox_targets_2d[...,8:10], layer_reg_weights_2d[...,8:10], avg_factor=max(num_pos_layer_2d[idx_layer], 1))
            ...
            if self.view_transform:
                ...
                # layer_loss_xy = self.loss_bbox(preds_view[...,:2], bbox_targets_view[...,:2], layer_reg_weights_view[...,:2], avg_factor=max(num_pos_layer_view[0], 1))
                # layer_loss_h = self.loss_bbox(preds_view[...,2:3], bbox_targets_view[...,2:3], layer_reg_weights_view[...,2:3], avg_factor=max(num_pos_layer_view[0], 1))
                # layer_loss_dim = self.loss_bbox(preds_view[...,3:6], bbox_targets_view[...,3:6], layer_reg_weights_view[...,3:6], avg_factor=max(num_pos_layer_view[0], 1))
                # layer_loss_rot = self.loss_bbox(preds_view[...,6:8], bbox_targets_view[...,6:8], layer_reg_weights_view[...,6:8], avg_factor=max(num_pos_layer_view[0], 1))
                # layer_loss_vel = self.loss_bbox(preds_view[..., 8:10], bbox_targets_view[...,8:10], layer_reg_weights_view[...,8:10], avg_factor=max(num_pos_layer_view[0], 1))
                ...
                # loss_dict['view_xy'] = layer_loss_xy.detach()
                # loss_dict['view_height'] = layer_loss_h.detach()
                # loss_dict['view_dim'] = layer_loss_dim.detach()
                # loss_dict['view_rot'] = layer_loss_rot.detach()
                # loss_dict['view_vel'] = layer_loss_vel.detach()
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False, for_roi=False):
        ...
        for id, preds_dict in enumerate(preds_dicts):
            ...
            if self.test_cfg['dataset'] == 'nuScenes':
                # self.tasks = [
                #     dict(num_class=1, class_names=['car'], indices=[0], radius=4),
                #     dict(num_class=1, class_names=['truck'], indices=[1], radius=12),
                #     dict(num_class=1, class_names=['construction_vehicle'], indices=[2], radius=12),
                #     dict(num_class=1, class_names=['bus'], indices=[3], radius=10),
                #     dict(num_class=1, class_names=['trailer'], indices=[4], radius=10),
                #     dict(num_class=1, class_names=['barrier'], indices=[5], radius=1),
                #     dict(num_class=1, class_names=['motorcycle'], indices=[6], radius=0.85),
                #     dict(num_class=1, class_names=['bicycle'], indices=[7], radius=0.85),
                #
            ...
            for i in range(batch_size):
                ...
                # cmask = temp[i]['cmask']
                #
                # nms_scores = copy.deepcopy(batch_score[i])
                # nms_scores = torch.max(nms_scores, dim=0)[0]
                # nms_scores[:self.num_proposals] += 5
                # nms_scores = nms_scores[cmask]

                # nms_scores = batch_score_max[i, cmask]
                    ...
                    for task in self.tasks:
                        ...
                        if task['radius'] > 0 and task_mask.sum() > 0:
                            if self.test_cfg['nms_type'] == 'circle':
                                # boxes_for_nms = torch.cat([boxes3d[task_mask][:, :2], nms_scores[:, None][task_mask]], dim=1)
                                ...
                                # top_scores = nms_scores[task_mask]
    ...
    def get_layer_num_proposal(self, idx_layer):
        if self.fuse_self and not self.lidar_query_only:
            if self.with_img and self.with_pts and idx_layer >= self.num_pts_decoder_layers:
                if self.double_gt:
                    if (idx_layer - self.num_pts_decoder_layers) % 2 == 0:
                        layer_num_proposal = self.num_proposals
                    else:
                        layer_num_proposal = self.num_img_proposals
                else:
