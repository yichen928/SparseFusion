...
class ImplicitHead2D_3D_Cross(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
                 pts_smca=True,
        ...
        super(ImplicitHead2D_3D_Cross, self).__init__()
        ...
        heads_depth = dict(proj_center=(2, 2), depth=(self.num_depth_class, 2))
        if extra_class_sup:
        ...
        if with_img:
            ...
            self.img_transformer = ImageTransformer2D_3D_Cross(
        ...
        if with_pts and with_img:
            if self.pe_norm == 'bn':
                ...
                # depth_key_pos_embed = PositionEmbeddingLearnedMultiInput([2, 12], hidden_channel)
            else:
            ...
            if self.pe_norm == 'bn':
                fusion_query_pos_embed = PositionEmbeddingLearned(2, hidden_channel)
                fusion_key_pos_embed = PositionEmbeddingLearned(2, hidden_channel)
            else:
                fusion_query_pos_embed = PositionEmbeddingLearnedwoNorm(2, hidden_channel)
                fusion_key_pos_embed = PositionEmbeddingLearnedwoNorm(2, hidden_channel)
            ...
            if self.initialize_by_heatmap and self.cross_heatmap:
                ...
                if cat_heatmap:
                    ...
                    #     nn.Conv2d(hidden_channel*2, hidden_channel, kernel_size=1),
                    #     nn.BatchNorm2d(hidden_channel),
                    #     nn.ReLU(inplace=True)
                    # )
        ...
        if self.initialize_by_heatmap:
            ...
            if img_heatmap_trick == 'centerpool':
                ...
                self.img_heatmap_head = self.build_heatmap(hidden_channel, bias, num_classes, layer_num=2)
                ...
                if activation == 'relu':
                    ...
                    raise NotImplementedError
                self.msca = MSCABlock(dim=hidden_channel, mlp_ratio=1, drop_path=0.1, norm_cfg=dict(type='BN2d'), act_layer=act_layer)
                self.img_heatmap_head = build_conv_layer(dict(type='Conv2d'), hidden_channel, num_classes, kernel_size=1, bias=bias)
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.with_img and self.with_pts: 
            ...
            normal_img_query_bboxes = img_ret_dicts[-1]['bbox'].detach().clone().permute(0, 2, 1)
            img_query_feat, img_query_pos_bev, depth_ret_dicts = self.depth_transformer(pts_query_feat, pts_query_pos, pts_query_height, img_query_feat, normal_img_query_pos, img_query_view, normal_img_query_bboxes, img_metas)
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, preds_dict):
        ...
        if self.with_img:
            res_tuple_2d = multi_apply(self.get_targets_single_2d, gt_bboxes, gt_labels, gt_img_centers_2d, list_of_pred_dict, np.arange(len(gt_bboxes)))
    ...
    def get_targets_single_2d(self, gt_bboxes, gt_labels, gt_centers_2d, preds_dict, batch_idx):
        ...
        if len(pos_inds) > 0:
            ...
            center_targets = center_targets[-self.num_proposals:]
            center_weights = center_weights[-self.num_proposals:]
            depth_labels = depth_labels[-self.num_proposals:]
            depth_weights = depth_weights[-self.num_proposals:]
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_pts_centers_2d, gt_img_centers_2d, preds_dicts, **kwargs):
        ...
        if self.with_img:
            if self.initialize_by_heatmap:
                ...
                labels_2d, label_weights_2d, bbox_targets_2d, bbox_weights_2d, center_targets_2d, center_weights_2d, \
                depth_2d, depth_weight_2d, ious_2d, num_pos_layer_2d, matched_ious_2d, heatmap_2d = self.get_targets(gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, preds_dicts[0])
            else:
                labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious, \
                labels_2d, label_weights_2d, bbox_targets_2d, bbox_weights_2d, center_targets_2d, center_weights_2d, \
                depth_2d, depth_weight_2d, ious_2d, num_pos_layer_2d, matched_ious_2d = self.get_targets(gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_img_centers_2d, preds_dicts[0])        # if hasattr(self, 'on_the_image_mask'):
        else:
            if self.initialize_by_heatmap:
