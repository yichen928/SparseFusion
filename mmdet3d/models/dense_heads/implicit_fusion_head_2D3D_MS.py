...
class ImplicitHead2D_3D_MS(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
                 pts_smca=False,
        ...
        super(ImplicitHead2D_3D_MS, self).__init__()
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.with_img:
            for i in range(self.level_num):
                img_inputs[i] = self.shared_conv_img(img_inputs[i])
            img_feat = img_inputs[0]  # [BS * n_views, C, H, W]
                ...
                if self.with_img and self.cross_heatmap:
                    ...
                    for level in range(self.level_num):
                        if self.cross_heatmap_stop_grad:
                            img_feat_cross.append(img_inputs[level].detach().clone())
                        else:
                            img_feat_cross.append(img_inputs[level].clone())
                else:
                    img_feat_cross = None
    ...
    def generate_heatmap_colattn(self, lidar_feat, img_feat, img_metas):
        ...
        for lvl in range(self.level_num):
            ...
            level_pos = self.level_pos[lvl].reshape(1, 1, self.hidden_channel, 1, 1)
            img_feat[lvl] = img_feat[lvl] + level_pos
