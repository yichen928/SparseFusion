...
from mmdet3d.models.utils import FFN, TransformerDecoderLayer, PositionEmbeddingLearned, PointTransformer3Donly, ImageTransformer3Donly, FusionTransformer3Donly, PointProjection, ImageProjection, ProjectionL2Norm, ProjectionLayerNorm

@HEADS.register_module()
class ImplicitHead3Donly(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
                 pts_smca=True,
        ...
        super(ImplicitHead3Donly, self).__init__()
        ...
        img_prediction_heads = FFN(hidden_channel, heads3d, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias)
        ...
        if with_pts:
            ...
            self.point_transformer = PointTransformer3Donly(
        ...
        if with_img:
            ...
            img_key_pos_embed = [PositionEmbeddingLearned(96, hidden_channel) for _ in range(num_img_decoder_layers)]
            self.img_transformer = ImageTransformer3Donly(
                img_smca=img_smca, hidden_channel=hidden_channel, num_heads=num_heads, num_decoder_layers=num_img_decoder_layers,
                out_size_factor_img=out_size_factor_img, num_views=num_views, prediction_heads=img_prediction_heads,
                ffn_channel=ffn_channel, dropout=dropout, activation=activation, test_cfg=test_cfg, bbox_coder=self.bbox_coder,
                query_pos=img_query_pos_embed, key_pos=img_key_pos_embed,
            )
        if with_pts and with_img:
            fusion_query_pos_embed = [PositionEmbeddingLearned(2, hidden_channel) for _ in range(num_fusion_decoder_layers)]
            # fusion_key_pos_embed = [PositionEmbeddingLearned(2, hidden_channel) for _ in range(num_fusion_decoder_layers)]
            ...
            self.fusion_transformer = FusionTransformer3Donly(
                ...
                test_cfg=test_cfg, query_pos=fusion_query_pos_embed, key_pos=fusion_query_pos_embed,
                pts_projection=pts_projection, img_projection=img_projection
            )
        ...
        if self.with_img:
            ...
            if self.initialize_by_heatmap:
                ...
                query_pos_realmetric = img_query_pos.permute(0, 2, 1) * self.test_cfg['out_size_factor'] * \
                                       self.test_cfg['voxel_size'][0] + self.test_cfg['pc_range'][
                                           0]  # [BS, 2, num_proposals]
                query_pos_3d = torch.cat([query_pos_realmetric, torch.ones_like(query_pos_realmetric[:, :1])],
                                         dim=1).detach().clone()  # [BS, 3, num_proposals]
                ...
                for sample_idx in range(batch_size):
                    ...
                    points = apply_3d_transformation(query_pos_3d[sample_idx].T, 'LIDAR', img_metas[sample_idx], reverse=True).detach()  # [num_proposals, 3]
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict, batch_idx):
        ...
        # else:
        #     num_layer = 1
        num_layer = self.num_pts_decoder_layers + self.num_img_decoder_layers + self.num_fusion_decoder_layers
        assign_result_list = []
        start = 0
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        if self.initialize_by_heatmap:
            labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious, heatmap = self.get_targets(gt_bboxes_3d, gt_labels_3d, preds_dicts[0])
        else:
            labels, label_weights, bbox_targets, bbox_weights, ious, num_pos_layer, matched_ious = self.get_targets(gt_bboxes_3d, gt_labels_3d, preds_dicts[0])
        ...
        num_layers = self.num_pts_decoder_layers + self.num_img_decoder_layers + self.num_fusion_decoder_layers
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False, for_roi=False):
        ...
        for id, preds_dict in enumerate(preds_dicts):
            layer_num_proposal = self.get_layer_num_proposal(self.num_pts_decoder_layers+self.num_img_decoder_layers+self.num_fusion_decoder_layers-1)
    ...
    def get_layer_num_proposal(self, idx_layer):
        if self.with_img and self.with_pts and idx_layer >= self.num_pts_decoder_layers+self.num_img_decoder_layers:
