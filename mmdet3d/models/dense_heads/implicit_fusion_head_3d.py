...
from mmdet3d.models.utils import PositionEmbeddingLearned, TransformerDecoderLayer, MultiheadAttention, FFN, PointTransformer3D, ImageTransformer3D, FusionTransformer3D
...
class ImplicitHead3D(nn.Module):
    def __init__(self,
                 with_pts=False,
                 with_img=False,
        ...
        super(ImplicitHead3D, self).__init__()
        ...
        if with_pts:
            self.point_transformer = PointTransformer3D(
        ...
        if with_img:
            assert learnable_query_pos or with_pts, "the query should be initialized with learning pos or pts heatmap"
            img_key_pos_embed = PositionEmbeddingLearned(96, hidden_channel)
            self.img_transformer = ImageTransformer3D(
                ...
                query_pos=query_pos_embed, key_pos=img_key_pos_embed
            )
        if with_pts and with_img:
            self.fusion_transformer = FusionTransformer3D(
        ...
        self.bev_pos = self.create_2D_grid(x_size, y_size)  # (1, N, 2)
        ...
        if self.initialize_by_heatmap:
            ...
            self.pts_query_pos = nn.Parameter(torch.rand([1, self.num_proposals, 2]), requires_grad=learnable_query_pos)
            self.pts_query_pos = inverse_sigmoid(self.pts_query_pos)
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        if self.with_pts:
            ...
            if self.initialize_by_heatmap:
                ...
                pts_query_pos = normal_bev_pos.gather(
                    index=top_proposals_index[:, None, :].permute(0, 2, 1).expand(-1, -1, normal_bev_pos.shape[-1]), dim=1)  # [BS, num_proposals, 2]
            else:
        ...
        if self.with_img:
            ...
            if self.initialize_by_heatmap:
                ...
                img_query_feat = nn.Parameter(torch.randn(batch_size, num_channel, self.num_proposals)).to(img_query_pos.device) # [BS, C, num_proposals]
                ...
                query_pos_realmetric = img_query_pos.permute(0, 2, 1).sigmoid() * (self.test_cfg['pc_range'][3] - self.test_cfg['pc_range'][0]) \
                                       + self.test_cfg['pc_range'][0]  # [BS, 2, num_proposals]
                ...
                for sample_idx in range(batch_size):
                    ...
                    img_crop_offset = (query_pos_3d.new_tensor(img_metas[sample_idx]['img_crop_offset'])
                    ...
                    points = apply_3d_transformation(query_pos_3d[sample_idx].T, 'LIDAR',
                                                     img_metas[sample_idx], reverse=True).detach()  # [num_proposals, 3]
                    ...
                    for view_idx in range(self.num_views):
                        ...
                        img_feat_on_mask = img_feat_collapsed[sample_idx, :, view_idx, center_xs]  # [C, num_on_image]
        ...
        for ret_dict in ret_dicts:
            ret_dict['center'][:, 0] = ret_dict['center'][:, 0].sigmoid() * (self.test_cfg['pc_range'][3] - self.test_cfg['pc_range'][0]) + self.test_cfg['pc_range'][0]
            ret_dict['center'][:, 1] = ret_dict['center'][:, 1].sigmoid() * (self.test_cfg['pc_range'][4] - self.test_cfg['pc_range'][1]) + self.test_cfg['pc_range'][1]
            ret_dict['height'] = ret_dict['height'].sigmoid() * (self.test_cfg['pc_range'][5] - self.test_cfg['pc_range'][2]) + self.test_cfg['pc_range'][2]
