...
class TransFusionHeadRay(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(TransFusionHeadRay, self).__init__()
        ...
        if self.fuse_img:
            ...
            self.decoder.append(
                TransformerDecoderLayer(
                    hidden_channel, num_heads, ffn_channel, dropout, activation,
                    self_posembed=PositionEmbeddingLearned(2, hidden_channel),
                    cross_posembed=PositionEmbeddingLearned(96, hidden_channel),
                ))
        ...
        if self.fuse_img:
            ...
            for sample_idx in range(batch_size if self.fuse_img else 0):
                ...
                for view_idx in range(self.num_views):
                    ...
                    query_pos_view = query_pos[sample_idx, on_the_image]
                    img_feat_pos_3d = self.img_pos_3d(img_feat_pos[0], lidar2img_rt[view_idx], img_scale_factor)
                    query_feat_view = self.decoder[self.num_decoder_layers](query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1, view_idx], query_pos_view[None], img_feat_pos_3d[None], attn_mask=attn_mask.log())
