...
class ImplicitFusionDetector2D_3D_MS(MVXTwoStageDetector):
    """Base class of Multi-modality VoxelNet."""

    def __init__(self, batch_scaling, pts_voxel_layer_height=None, **kwargs):
        super(ImplicitFusionDetector2D_3D_MS, self).__init__(**kwargs)
    ...
    def init_weights(self, pretrained=None):
        ...
        super(ImplicitFusionDetector2D_3D_MS, self).init_weights(pretrained)
    ...
    #     max_num_points = self.pts_voxel_layer.max_num_points
    #
    #     height_num = grid_size[2]
    #     x_num = grid_size[0] // out_size_factor
    #     y_num = grid_size[1] // out_size_factor
    #     num_points = max_num_points * out_size_factor * out_size_factor * height_num
    #
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        # voxel_height = self.extract_voxel_heights(voxels, coors)
        ...
        x[0] = torch.cat([x[0], voxel_height], dim=1)
        return x
    ...
    def forward_train(self,
        ...
        if pts_feats:
            losses_pts = self.forward_pts_train(
                pts_feats, img_feats, gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_pts_centers_2d, gt_img_centers_2d, img_metas, gt_bboxes_ignore
            )
        ...
        # if index % 10 == 0:
        #     id = index // 10
        #     torch.save(points, "vis/%d_points.pt"%id)
        #     torch.save(img, "vis/%d_img.pt"%id)
        #     torch.save(gt_bboxes_3d, "vis/%d_gt_bboxes_3d.pt"%id)
        #     torch.save(gt_labels_3d, "vis/%d_gt_labels_3d.pt"%id)
        #     torch.save(gt_bboxes, "vis/%d_gt_bboxes_2d.pt"%id)
        #     torch.save(gt_labels, "vis/%d_gt_labels_2d.pt"%id)
        #     torch.save(gt_pts_centers_2d, "vis/%d_gt_pts_centers_2d.pt"%id)
        #     torch.save(gt_img_centers_2d, "vis/%d_gt_img_centers_2d.pt"%id)
        #
        #     import pickle
        #     with open("vis/%d_img_metas.pkl"%id, "wb") as file:
    ...
    def forward_pts_train(self,
        ...
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, gt_bboxes, gt_labels, gt_pts_centers_2d, gt_img_centers_2d, img_metas, outs]
