...
class ImageTransformer2D_3D(nn.Module):
    def __init__(self, num_views, hidden_channel, num_heads, num_decoder_layers, prediction_heads, img_smca,
        ...
        super(ImageTransformer2D_3D, self).__init__()
    ...
    def img_pos_3d(self, img_pos, lidar2img, img_scale_factor, D=32):
        ...
        coords3d[..., 0:1] = coords3d[..., 0:1] * self.grid_x_size
        coords3d[..., 1:2] = coords3d[..., 1:2] * self.grid_y_size

        coords3d = coords3d.contiguous().view(coords3d.size(0), D*3)
        return coords3d

class PointTransformer2D_3D(nn.Module):
    def __init__(self, hidden_channel, num_heads, num_decoder_layers, prediction_heads, pts_smca, ffn_channel, dropout, activation, test_cfg, query_pos, key_pos, supervision3d):
    ...
    def forward(self, pts_query_feat, pts_query_pos, lidar_feat_flatten, bev_pos):
        ...
        for i in range(self.num_decoder_layers):
            ...
            # Prediction
            res_layer = self.prediction_heads(pts_query_feat)
            if self.supervision3d:
                res_layer['center'] = res_layer['center'] + pts_query_pos.permute(0, 2, 1)
            else:
                res_layer['center'] = pts_query_pos.permute(0, 2, 1)
...
class FusionTransformer2D_3D(nn.Module):
    def __init__(self, hidden_channel, num_heads, num_decoder_layers, prediction_heads, ffn_channel, dropout, activation, test_cfg,
                 query_pos, key_pos, pts_projection=nn.Identity(), img_projection=nn.Identity(), normal_pos=False):
        super(FusionTransformer2D_3D, self).__init__()
        self.hidden_channel = hidden_channel
        self.num_heads = num_heads
        self.num_decoder_layers = num_decoder_layers
        self.prediction_heads = prediction_heads
    ...
    def forward(self, pts_query_feat, pts_query_pos, img_query_feat, img_query_pos):
        ...
        for i in range(self.num_decoder_layers):
            ...
            if pts_query_pos.shape[-1] == 3:
                pts_query_pos_2d = pts_query_pos[..., :2]
                pts_query_pos_height = pts_query_pos[..., 2:]
            else:
                pts_query_pos_2d = pts_query_pos

            if self.normal_pos:
                scale = torch.tensor([self.grid_x_size, self.grid_y_size]).reshape(1, 1, 2).to(pts_query_pos_2d.device)
                denormal_pts_query_pos = (pts_query_pos_2d + 1) / 2 * scale
                res_layer['center'] = res_layer['center'] + denormal_pts_query_pos.permute(0, 2, 1)
            else:
            ...
            if pts_query_pos.shape[-1] == 3:
                if self.normal_pos:
                    denormalize_height = (pts_query_pos_height + 1) / 2 * (self.test_cfg['pc_range'][5]-self.test_cfg['pc_range'][2]) + self.test_cfg['pc_range'][2]
                    res_layer['height'] = res_layer['height'] + denormalize_height.permute(0, 2, 1)
                else:
                    res_layer['height'] = res_layer['height'] + pts_query_pos_height.permute(0, 2, 1)
                pts_query_pos[..., :2] = res_layer['center'].detach().clone().permute(0, 2, 1)
                pts_query_pos[..., 2:3] = res_layer['height'].detach().clone().permute(0, 2, 1)
                if self.normal_pos:
                    pts_query_pos[...,:2] = (pts_query_pos[...,:2] / scale) * 2 - 1
                    pts_query_pos[..., 2] = (pts_query_pos[..., 2] - self.test_cfg['pc_range'][2]) / (self.test_cfg['pc_range'][5] - self.test_cfg['pc_range'][2]) * 2 - 1
            else:
                pts_query_pos = res_layer['center'].detach().clone().permute(0, 2, 1)
                if self.normal_pos:
                    pts_query_pos = (pts_query_pos / scale) * 2 - 1

        return pts_query_feat, pts_query_pos, ret_dicts


class FusionTransformer2D_3D_Fuse(nn.Module):
    def __init__(self, hidden_channel, num_heads, num_decoder_layers, prediction_heads, ffn_channel, dropout, activation, test_cfg,
                 pts_projection=nn.Identity(), img_projection=nn.Identity(), normal_pos=False):
        super(FusionTransformer2D_3D_Fuse, self).__init__()
        self.hidden_channel = hidden_channel
        self.num_heads = num_heads
        self.num_decoder_layers = num_decoder_layers
        self.prediction_heads = prediction_heads
    ...
    def forward(self, pts_query_feat, pts_query_pos, img_query_feat, img_query_pos):
        ret_dicts = []
        pts_query_feat = self.pts_projection(pts_query_feat, pts_query_pos)
        img_query_feat = self.img_projection(img_query_feat, img_query_pos)
        fuse_query_feat = torch.cat([img_query_feat, img_query_feat], dim=2)
        for i in range(self.num_decoder_layers):
            # Transformer Decoder Layer
            # :param query: B C Pq    :param query_pos: B Pq 3/6
            fuse_query_feat = self.decoder[i](fuse_query_feat, fuse_query_feat, None, None)

            # Prediction
            res_layer = self.prediction_heads(fuse_query_feat)
            ret_dicts.append(res_layer)

        return pts_query_feat, None, ret_dicts
