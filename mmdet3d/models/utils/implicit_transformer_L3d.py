...
class ImageTransformerL3D(nn.Module):
    def __init__(self, num_views, hidden_channel, num_heads, num_decoder_layers, prediction_heads, img_smca,
        ...
        super(ImageTransformerL3D, self).__init__()
        ...
        self.img_x_size = test_cfg['img_scale'][0] / test_cfg['img_scale_factor']
        self.img_y_size = test_cfg['img_scale'][1] / test_cfg['img_scale_factor']
        ...
        for i in range(self.num_decoder_layers):
            ...
            for sample_idx in range(batch_size):
                ...
                for view_idx in range(self.num_views):
                    ...
                    # normal_img_query_pos_view = normalize_pos(img_query_pos_view[None], self.img_x_size, self.img_y_size)
                    ...
                    if self.img_smca:
                        ...
                        # normal_img_query_pos = normalize_pos(img_query_pos, self.grid_x_size, self.grid_y_size)

                        img_query_feat_view = self.decoder[i](
                            img_query_feat_view[None], img_feat_flatten[sample_idx:sample_idx + 1,view_idx],
                            img_query_pos[sample_idx:sample_idx+1, on_the_image], img_feat_pos_view_3d[None], attn_mask=attn_mask.log()
                        )
                    else:
                        # normal_img_query_pos = normalize_pos(img_query_pos, self.grid_x_size, self.grid_y_size)
                        img_query_feat_view = self.decoder[i](
                            img_query_feat_view[None], img_feat_flatten[sample_idx:sample_idx+1, view_idx],
                            img_query_pos[sample_idx:sample_idx + 1, on_the_image], img_feat_pos_view_3d[None]
                        )
    ...
    def img_pos_3d(self, img_pos, lidar2img, D=32):
        ...
        coords3d[...,:2] = inverse_sigmoid(coords3d[...,:2])

        coords3d[...,:2] = denormalize_pos(coords3d[...,:2], self.grid_x_size, self.grid_y_size)
        coords3d = coords3d.contiguous().view(coords3d.size(0), D*3)
        return coords3d


class FusionTransformerL3D(nn.Module):
    def __init__(self, hidden_channel, num_heads, num_decoder_layers, prediction_heads, ffn_channel, dropout, activation, test_cfg, query_pos, key_pos):
        super(FusionTransformerL3D, self).__init__()
        self.hidden_channel = hidden_channel
        self.num_heads = num_heads
        self.num_decoder_layers = num_decoder_layers
        self.prediction_heads = prediction_heads
    ...
    def forward(self, pts_query_feat, pts_query_pos, img_query_feat, img_query_pos):
        ...
        all_query_pos = pts_query_pos
        all_query_feat = torch.cat([pts_query_feat, img_query_feat], dim=1)
        ret_dicts = []
        for i in range(self.num_decoder_layers):
            ...
            # res_layer['center'] = res_layer['center'] + denormalize_pos(all_query_pos, self.grid_x_size, self.grid_y_size).permute(0, 2, 1)
            ...
            # all_query_pos = normalize_pos(all_query_pos, self.grid_x_size, self.grid_y_size)

        return all_query_feat, all_query_pos, ret_dicts


class PointTransformerL3D(nn.Module):
    def __init__(self, hidden_channel, num_heads, num_decoder_layers, prediction_heads, pts_smca, ffn_channel, dropout, activation, test_cfg, query_pos, key_pos):
        super(PointTransformerL3D, self).__init__()
        self.hidden_channel = hidden_channel
        self.num_heads = num_heads
        self.num_decoder_layers = num_decoder_layers
        self.prediction_heads = prediction_heads
    ...
    def forward(self, pts_query_feat, pts_query_pos, lidar_feat_flatten, bev_pos):
        ...
        # pts_query_pos = normalize_pos(pts_query_pos, self.grid_x_size, self.grid_y_size)
        for i in range(self.num_decoder_layers):
            # Transformer Decoder Layer
            # :param query: B C Pq    :param query_pos: B Pq 3/6
            if self.pts_smca:
            ...
            # Prediction
            res_layer = self.prediction_heads(pts_query_feat)
            # res_layer['center'] = res_layer['center'] + denormalize_pos(pts_query_pos, self.grid_x_size, self.grid_y_size).permute(0, 2, 1)
