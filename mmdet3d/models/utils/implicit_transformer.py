...
class ImageTransformer(nn.Module):
    ...
    def forward(self, img_query_feat, img_query_pos, img_feat_flatten, img_feat_pos, img_metas):
        normal_img_feat_pos = normalize_pos(img_feat_pos, self.img_x_size, self.img_y_size)
                ...
                for view_idx in range(self.num_views):
                    ...
                    if self.img_smca:
                        ...
                        img_query_feat_view = self.decoder[i](
                            ...
                            normal_img_query_pos_view, normal_img_feat_pos, attn_mask=attn_mask.log()
                        )
                    else:
                        img_query_feat_view = self.decoder[i](
                            img_query_feat_view[None], img_feat_flatten[sample_idx:sample_idx+1, view_idx],
                            normal_img_query_pos_view, normal_img_feat_pos
                        )
